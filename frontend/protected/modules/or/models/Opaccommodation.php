...
/**
 * This is the model class for table "seg_opaccommodation".
 *
 * The followings are the available columns in table 'seg_opaccommodation':
 ...
 * @property OpaccommodationDetails[] $opaccommodationDetails
 */
class Opaccommodation extends CActiveRecord
{
	/**
	...
	public function relations()
	{
		...
		return array(
			'opaccommodationDetails' => array(self::HAS_MANY, 'OpaccommodationDetails', 'refno'),
		);
	}
	 ...
	 * @return Opaccommodation the static model class
	 */
	...
	public function getEntry($enc_no){
		...
		$criteria->with = array(
			'opaccommodationDetails' => array('joinType' => 'INNER JOIN'),
		);

		$criteria->addSearchCondition('encounter_nr', $enc_no, true, 'AND');
		
		$criteria->order = "opaccommodationDetails.entry_no DESC";
		$result = $this->find($criteria);

		if($result)
			return $result->opaccommodationDetails->entry_no + 1;
