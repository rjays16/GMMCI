...
class smarty_care extends Smarty {
	...
	*/
	function smarty_care ($dirname, $bInit = TRUE, $bShowCopy = TRUE, $bLoadJS = TRUE) {
 		...
 		global $root_path, $templatedir, $default_template, $sDocRoot, $LDCloseAlt, $cfg, $lang, $pgt, $GLOBAL_CONFIG, $QuickMenu;
	...
	function InitializeGUI(){
 		global $root_path, $lang, $cfg, $QuickMenu, $breakfile;
		...
		if ($QuickMenu) {
			foreach ($QuickMenu as $i=>$v) {
				$QuickMenu[$i]['icon'] = createComIcon($root_path,$v['icon'],'0');
				$QuickMenu[$i]['url'] = str_replace('{{$URL_APPEND}}',URL_APPEND."&userck=$userck",$v['url']);
			}
			$QuickMenu[] = array(
				'icon'=> createComIcon($root_path,'close.gif','0'),
				'url' => $breakfile,
				'label' => 'Close');
			
			$this->assign('QuickMenu', $QuickMenu);
			$this->assign('URL_APPEND', URL_APPEND."&userck=$userck");
		}
		
			$this->assign('sRootPath', $root_path);
	}
