...
$smarty->assign('sToolbarTitle',"Supplies::$target External Request");
...
# Window bar title
$smarty->assign('sWindowTitle',"Requisition::$target External Request");
...
?>
    ...
    function openItemsTray() {
        ...
        var url = 'seg-supply-office-request-tray.php';
        overlib(
...
//            $dept_nr = '';
    ...
    $ori_area = '<select class="jedInput" name="ori_area" id="ori_area" >'."\n".$ori_area."</select>\n".
                "<input type=\"hidden\" id=\"area2\" name=\"area2\" value=\"".$_GET['ori_area']."\"/>";
    $smarty->assign('sSelectArea',$ori_area);    
    
    //dest
    /*   
    $result = $objsupplier->getSupplierOptions();
    ...
    */
    $result = $objsupplier->getAllSuppliers(); 
    $count = 0;    
    $dest_area = '';
    if ($result) {
        while($row=$result->FetchRow()){
            $checked=($row['supplier_id']==$_GET['des_area']) || ($row['supplier_id'] == $_POST['des_area']) ? 'selected="selected"' : "";
            $dest_areas .= "<option value=\"".$row['supplier_id']."\" $checked>".$row['name']."</option>\n";
            if ($checked) $index = $count;
            $count++;
        }
        $dest_area = '<select class="jedInput" name="des_area" id="des_area" >'."\n".$dest_areas."</select>\n".
    ...
    $lastnrthis = $objextreq->getLastNr(date("Y-m-d")); 
    ...
    //$smarty->assign('sRefNo','<input class="jedInput" id="refno" name="refno" type="text" size="10" value="'.((($submitted && !$saveok)) ? $_POST["refno"] : $lastnrthis).'" style="font:bold 12px Arial"/>');                                                  
    ...
    $smarty->assign('sExtRemarks','<textarea class="segInput" id="remarks" name="remarks" cols="100" rows="2" style="float:left; margin-left:3px;margin-top:3px">'.($submitted && !$saveok ? $_POST['remarks'] : $remarks).'</textarea>');           
...
$smarty->assign('sMainBlockIncludeFile','supply_office/supply-external-request-form.tpl');
