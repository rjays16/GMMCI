...
//    define('__DEBUG_MODE',1);
    ...
    function populateProductList($sElem,$page,$keyword,$type,$area = '') {
        global $db;
        ...
        if ($ergebnis) {            
            ...
            while($result=$ergebnis->FetchRow()) {
                $avgdet = $pc->getExtendedProductInfo($result["bestellnum"]);
                $skuInventory = new SkuInventory();

                //$invobj->getInventoryAtHandbyDateWithSerial();
               ...
               // $details->avg = $skuInventory->getsupplierprice($result["bestellnum"]);
                ...
                $details->qtypack = $avgdet["qty_per_pack"] ? $avgdet["qty_per_pack"] : 1;
               
                $details->perpack = $skuInventory->getItemQty($result["bestellnum"],'',$area); //modified by julz
                $details->supplier_price = $result["supplier_price"];
