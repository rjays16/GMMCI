...
//    }
    ...
    function goAddItem($items, $unitids, $is_pcs, $qtys, $areaFrom)  {
        global $db;
            ...
            if ($result) {
                if ($result->RecordCount()) {
                    if ($row = $result->FetchRow()) {
                        ...
                        $obj->avg = $skuInventory->getItemAvgCost($item, '', $areaFrom);
                        $obj->perpack = $extendedrow['qty_per_pack'] ? $extendedrow['qty_per_pack'] : 1;
...
//    }
/**
 * Only Inventory Areas
 * Edited by Marc Lua
 * @param $s_areacode
 * @param string $r_areacode
