...
//echo "user: ".$_SESSION['sess_user_personell_nr']."<br>destination: ".$_REQUEST['destination']."<br>";
$areaFrom = @$_GET['source'];
...
if (isset($_POST["submitted"])) {
    ...
    foreach ($_REQUEST['itemcode'] as $i => $v) {
        ...
        if ($_REQUEST['acknowledge'][$i] == 'approve') {
            $avgCost = $skuobj->getItemAvgCost($v, '', $areaFrom);
            $issue_obj->acknowledgeIssuance($_REQUEST['refno'], $_SESSION['sess_user_personell_nr'], $_REQUEST['itemcode'][$i]);
            $inventory_obj->setInventoryParams($_REQUEST['itemcode'][$i], $_REQUEST['destination'], $_REQUEST['refno'], ISSUANCE);
            if ($_REQUEST['expiry'][$i] == "-" && $_REQUEST['serial'][$i] != "-") {
                //$inventory_obj->setSerialArray('00', 0, '0000-00-00', '1');
                $inventory_obj->addInventory($_REQUEST['qty'][$i], $_REQUEST['unitid'][$i], NULL, $_REQUEST['serial'][$i], NULL, $avgCost);
            } else if ($_REQUEST['serial'][$i] == "-" && $_REQUEST['expiry'][$i] != "-") {
                $inventory_obj->addInventory($_REQUEST['qty'][$i], $_REQUEST['unitid'][$i], $_REQUEST['expiry'][$i], NULL, NULL, $avgCost);
            } else {
                $inventory_obj->addInventory($_REQUEST['qty'][$i], $_REQUEST['unitid'][$i], NULL, NULL, NULL, $avgCost);
            }
        } else if($_REQUEST['acknowledge'][$i] == 'cancel')  {
            $issue_obj->acknowledgeIssuance($_REQUEST['refno'], $_SESSION['sess_user_personell_nr'], $_REQUEST['itemcode'][$i]);
            $isBigUnit = $unit->isUnitIDBigUnit($_REQUEST['unitid'][$i]);
            $qty = $_REQUEST['qty'][$i];
            /* if($isBigUnit) {
                $qty *=  $item->getQtyPerBigUnit($v);
            }*/
            $issue_obj->cancelRequestServed($_REQUEST['refno'], $v, $qty);
            $invHelper->addStock($v, $areaFrom, $qty, '', $_REQUEST['refno'], $_REQUEST['refno'], PHRETURN, $_REQUEST['unitid'][$i]);
        }
...
if($resultDetails!=NULL){
    ...
    while($issueRow = $resultDetails->fetchRow()){
        ...
        $itemnameAdd[$i] = addslashes($row['artikelname']);
...
$smarty->assign('sFormStart','<form ENCTYPE="multipart/form-data" action="'.$thisfile.URL_APPEND."&clear_ck_sid=".$clear_ck_sid.$qs.'&source='.$_GET['source'].'" method="POST" id="orderForm" name="inputform" onSubmit="return validate()">');
...
$smarty->assign('sApproveAll', '<button style="cursor:pointer" class="jedButton" onclick="return selectAll(\'approve\');">Approve All</button>');
$smarty->assign('sDenyAll', '<button class="jedButton" style="cursor:pointer" onclick="return selectAll(\'cancel\');">Cancel All</button>');
