...
$areaname = $db->GetOne("SELECT area_name FROM seg_pharma_areas WHERE area_code='".$info['pharma_area']."'");
   ...
   $stype = "PHARMACY ORDER REQUEST ($type)";
...
$result = $order_obj->getOrderItemsFullInfo($_REQUEST['ref']);
   ...
   $infoResult3 = $order_obj->get_create_person($info['refno']);
    if ($infoResult3)    $info3 = $infoResult3->FetchRow();


   $infoResult4 = $order_obj->get_modify_person($info['refno']);
...
$total = 0;
        ...
        $amount = $res['force_price'] * $res['quantity'];
        // $data[0]['amount']  = number_format($amount,2);
        $total += $amount;

             $data[$i] = array('item_no'=>$res['bestellnum'],
                              'desc'=>$res['artikelname'],
                              'price'=>number_format($res['force_price'],2),
                              'qty'=>$res['quantity'],
        ...
        $infoResult4 = $order_obj->getinsurance($info['pid']);
...
#added by daryl
     ...
     $infoResult2 = $order_obj->getpatientinfo($info['pid'], $info['encounter_nr']);
...
showReport('PharmacyReports',$params,$data,"PDF");
