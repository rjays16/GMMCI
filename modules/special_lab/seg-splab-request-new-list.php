 ...
 */
 ...
 $smarty->assign('sToolbarTitle',"$LDSpecialLab :: List of Service Requests");
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"$LDSpecialLab :: List of Service Requests");
...
function deleteRequest(refno){
	var answer = confirm("Are you sure you want to delete the special laboratory request with a reference no. "+(refno)+"?");
		if (answer){
...
function ShortcutKeys(){
	...
	function RequestList(){
		...
		urlholder="labor_test_request_pass.php?sid=<?=$sid?>&lang=<?=$lang?>&target=specialLab_result&user_origin=splab&done=1";
		window.location.href=urlholder;
	}
...
function addPerson(listID, refno, name, req_date, urgency, labstatus, paid, encounter_nr,pid,age,sex,location,enctype,or_no,is_cash) {
	...
	if (list) {
		...
		if (refno) {
			...
			//855, 440
			editlink ='onclick="return overlib(OLiframeContent(\'seg-splab-request-new.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&user_origin=splab&popUp='+1+'&ref='+refno+'&encounter_nr='+encounter_nr+'&pid='+pid+'\', 800, 440, \'flab-list\', 1, \'auto\'), ' +
