...
require('./roots.php');
require($root_path."modules/dialysis/ajax/dialysis-transaction.common.php");
...
<?php
	$varArray = array(
		...
		'var_civil_status'=>'', #added July 21, 2010
		'var_dob'=>'', #added July 21, 2010
		'var_photo_filename'=>'', #added July 22, 2010
		'var_type'=>'' #added July 21, 2010
		#end cha
	);
...
function startAJAXSearch(searchID, page) {
	...
	var from_dialysis = '<?=$_GET["from_dialysis"]?>';	//added by cha, july 22,2010
	...
	if (searchEL && searchEL.value.length >= 3) {
		...
		//modified by cha, july 22, 2010
		if(from_dialysis=="1")
			AJAXTimerID = setTimeout("xajax_populatePersonList('"+searchID+"','"+searchText+"',"+page+","+searchLastname+","+includeEnc+",0,0,'"+from_dialysis+"')",100);
		else
...
<script type="text/javascript" src="js/person-search-gui.js?t=<?=time()?>"></script>
