...
var globalActiveDHTMLGridObject;String.prototype._dhx_trim = function(){return this.replace(/&nbsp;/g," ").replace(/(^[ \t]*)|([ \t]*$)/g,"");};function dhtmlxArray(ar){return dhtmlXHeir((ar||new Array()),new _dhtmlxArray());};function _dhtmlxArray(){return this;};_dhtmlxArray.prototype._dhx_find = function(pattern){for(var i=0;i<this.length;i++){if(pattern==this[i])return i;};return -1;};_dhtmlxArray.prototype._dhx_delAt = function(ind){if(Number(ind)<0 || this.length==0)
 ...
 {
	 this.entBox = document.createElement("DIV");this.entBox.id = "cgrid2_"+(new Date()).getTime();};
	 this.dhx_Event();
	 ...
	 this.loadedKidsHash = null;this.UserData = new Array(0)
 ...
 this.hdrSizeA=25;
 else this.hdrSizeA=200;
 this.hdrBox.style.height=this.hdrSizeA+"px";
 if (_isIE)this.hdrBox.style.overflowX="hidden";
 else
 ...
 this.pagingOn = false;
 this.rowsBufferOutSize = 0;dhtmlxEvent(window,"unload",function(){try{self.destructor();}catch(e){}});
 this.loadXML = function(url,afterCall){if (this._dload){this._dload=url;
 this._askRealRows(null,afterCall);return true;};if (this._xmlaR)this.setXMLAutoLoading(url);if(url.indexOf("?")!=-1)
 var s = "&";else
 ...
 this._srdh=22;this._sizeFix=1;this._borderFix=(_isIE?1:0);break;break;case "gray": if ((_isIE)&&(document.compatMode != "BackCompat")) this._srdh=22;this._sizeFix=1;this._borderFix=(_isIE?1:0);break;};this.setSizes();};this.doLoadDetails = function(obj){var root = self.xmlLoader.getXMLTopNode(self._tttag)
 ...
 };var self=this;var arrTS=new Array();var atype = this.cellType[col];var amet="getValue";if (atype=="link")amet="getContent";for (var i=0;i<this.rowsCol.length;i++)arrTS[this.rowsCol[i].idd]=this.cells3(this.rowsCol[i],col)[amet]();this._sortRows(col,type,order,arrTS);};this._sortRows = function(col,type,order,arrTS){var sort="sort";if (this._sst)sort="stablesort";if(type=='str'){this.rowsCol[sort](function(a,b){if(order=="asc")return arrTS[a.idd]>arrTS[b.idd]?1:-1
 else
 ...
 for(var i=0;i<r.childNodes.length;i++){var pfix="";if (_isIE)r.childNodes[i].style.cssText = pfix+"width:"+r.childNodes[i].style.width+";"+styleString;else
 ...
 if (!r)return;if (ind<r.childNodes.length){var pfix="";if (_isIE)r.childNodes[ind].style.cssText = pfix+"width:"+r.childNodes[ind].style.width+";"+styleString;else
 ...
 };};this.getUserData = function(row_id,name){if(row_id=="")row_id = "gridglobaluserdata";var z=this.UserData[row_id];return (z?z.get(name):"");};this.setEditable = function(fl){if(fl!='true' && fl!=1 && fl!=true)ifl = true;else
 ...
 for(var i=0;i<this.selectedRows.length;i++){var r=this.rowsAr[this.selectedRows[i].idd];r.className=r.className.replace(/rowselected/g,"");};this.selectedRows = new dhtmlxArray(0)
 ...
 };};this.setOnRowSelectHandler = function(func,anyClick){this.attachEvent("onRowSelect",func);this._chRRS=(!convertStringToBoolean(anyClick));};this.setOnScrollHandler = function(func){this.attachEvent("onScroll",func);};this.setOnEditCellHandler = function(func){this.attachEvent("onEditCell",func);};this.setOnCheckHandler = function(func){this.attachEvent("onCheckbox",func);};this.setOnEnterPressedHandler = function(func){this.attachEvent("onEnter",func);};this.setOnBeforeRowDeletedHandler = function(func){this.attachEvent("onBeforeRowDeleted",func);};this.setOnRowAddedHandler = function(func){this.attachEvent("onRowAdded",func);};this.setOnGridReconstructedHandler = function(func){this.attachEvent("onGridReconstructed",func);};dhtmlXGridObject.prototype.setOnResize=function(func){this.attachEvent("onResize",func);};dhtmlXGridObject.prototype.setOnResizeEnd=function(func){this.attachEvent("onResizeEnd",func);};this.getPosition = function(oNode,pNode){if(!pNode)var pNode = document.body
 ...
 };if (xstyle)this.setRowTextStyle(rId,xstyle);this.callEvent("onRowCreated",[r.idd,r,xml]);};dhtmlXGridObject.prototype._fillRowFromXML=function(r,xml,tree,pId){var cellsCol = this.xmlLoader.doXPath("./cell",xml);var strAr = new Array(0);for(var j=0;j<cellsCol.length;j++){var cellVal=cellsCol[j];var exc=cellVal.getAttribute("type");if (cellVal.firstChild)cellVal=cellVal.firstChild.data;else cellVal="";if (j!=tree)strAr[strAr.length] = cellVal;else
 strAr[strAr.length] = [pId,cellVal,((xml.getAttribute("xmlkids")||r._xml)?"1":"0"),(cellsCol[j].getAttribute("image")||"leaf.gif")];if (exc)r.childNodes[j]._cellType=exc;};if (this._c_order)strAr=this._swapColumns(strAr);for(var j=0;j<cellsCol.length;j++){css1=cellsCol[j].getAttribute("class");if (css1)r.childNodes[j].className+=" "+css1;};this._fillRow(r,strAr);if ((r.parentNode)&&(r.parentNode.tagName))
 this._postRowProcessing(r,xml);return r;};dhtmlXGridObject.prototype._innerParse=function(rowsCol,startIndex,tree,pId,i){i=i||0;var imax=i+this._ads_count*1;var r=null;var rowsCol2;for(var i;i<rowsCol.length;i++){if ((pId)||(i<this.rowsBufferOutSize || this.rowsBufferOutSize==0)){this._parsing_=true;var rId = (rowsCol[i].getAttribute("id")||(this.rowsCol.length+2));r=this._prepareRow(rId);if (tree!=-1){rowsCol2 = this.xmlLoader.doXPath("./row",rowsCol[i]);if ((rowsCol2.length!=0)&&(this._slowParse))
