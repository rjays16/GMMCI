...
var $t_male_total, $t_female_total, $t_total;                   
   ...
   function RepGen_CausesOfMortality ($from, $to, $dept_nr) {
    ...
    function Header() {
        ...
        $this->Cell($this->ColumnWidth[2] + $this->ColumnWidth[3], $row, "", "TLR", 0);
        $this->Cell($this->ColumnWidth[4] + $this->ColumnWidth[5], $row, "", "TLR", 0);
        $this->Cell($this->ColumnWidth[6] + $this->ColumnWidth[7], $row, "", "TLR", 0);
        ...
        $this->Cell($this->ColumnWidth[2] + $this->ColumnWidth[3], $row, "0 - <7 days old", "LRB", 0, 'C');
        $this->Cell($this->ColumnWidth[4] + $this->ColumnWidth[5], $row, "7 - 14 days old", "LRB", 0, 'C');
        $this->Cell($this->ColumnWidth[6] + $this->ColumnWidth[7], $row, "15 - 28 days old", "LRB", 0, 'C');
        $this->Cell($this->ColumnWidth[8] + $this->ColumnWidth[9] + $this->ColumnWidth[10], $row, "", 0, 0, 'C');
        $this->Cell($this->ColumnWidth[11], $row, "TABULATION", "LR", 1, 'C');
        ...
        $this->Cell($this->ColumnWidth[11], $row, "LIST", "LRB", 0, 'C');
   ...
   if ($this->dept_nr){
     ...
     $sql = "SELECT cd.code AS subcode, 
        ...
        SUM(CASE WHEN p.sex='m' AND (floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))<7 OR floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age)) IS NULL) then 1 else 0 end) AS male_below7, 
        SUM(CASE WHEN p.sex='f' AND (floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))<7 OR floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age)) IS NULL) then 1 else 0 end) AS female_below7, 
        SUM(CASE WHEN p.sex='m' AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))>=7 AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))<=14 then 1 else 0 end) AS male_7to14, 
        SUM(CASE WHEN p.sex='f' AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))>=7 AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))<=14 then 1 else 0 end) AS female_7to14, 
        SUM(CASE WHEN p.sex='m' AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))>=15 AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))<=28 then 1 else 0 end) AS male_15to28, 
        SUM(CASE WHEN p.sex='f' AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))>=15 AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))<=28 then 1 else 0 end) AS female_15to28, 

        SUM(CASE WHEN p.sex='m' AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))<29 then 1 else 0 end) AS male_total, 
        SUM(CASE WHEN p.sex='f' AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))<29 then 1 else 0 end) AS female_total, 

        SUM(CASE WHEN (p.sex='f' OR p.sex = 'm') AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))<29 then 1 else 0 end) AS total 
