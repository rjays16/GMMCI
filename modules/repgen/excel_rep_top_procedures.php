<?php
	...
	//Statistics of Top Procedures (Excel)
	...
	class Excel_Stat_TopProcedure extends Spreadsheet_Excel_Writer{
		var $from, $to;

				function Excel_Stat_TopProcedure($from, $to){
					...
					$this->Headers = array('RANK', 'OCCURRENCE', 'PHIC', 'NON-PHIC', 'ICP CODE', 'OPERATIONS PERFORMED');
	...
	$rep = new Excel_Stat_TopProcedure($_GET['from'],$_GET['to']);
	$rep->ExcelHeader();
	$rep->FetchData();
	$rep->send('excel_rep_top_procedures.xls');
	$rep->close();
?>
