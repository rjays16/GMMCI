...
var $t_male_total, $t_female_total, $t_total;                   
   ...
   function RepGen_CausesOfMortality ($from, $to, $dept_nr) {
    ...
    function Header() {
        ...
        $this->SetTextColor(0);
        $row=4;
        $age_width = $this->ColumnWidth[2] + $this->ColumnWidth[3] + $this->ColumnWidth[4] + $this->ColumnWidth[5];
        $age_width += $this->ColumnWidth[6] + $this->ColumnWidth[7] + $this->ColumnWidth[8] + $this->ColumnWidth[9]; 
        
        $this->Cell($this->ColumnWidth[0]+$this->ColumnWidth[1], $row, "", "TLR", 0);
        $this->Cell($age_width, $row, "Number of Cases by Age", 1, 0, 'C');
        $this->Cell($this->ColumnWidth[10] + $this->ColumnWidth[11] + $this->ColumnWidth[12], $row, "", "TLR", 0);
        $this->Cell($this->ColumnWidth[13], $row, "ICD 10 CODE/", "TLR", 1, 'C');
        
        $this->Cell($this->ColumnWidth[0] + $this->ColumnWidth[1], $row, "Conditions", "LR", 0, 'C');
        $this->Cell($this->ColumnWidth[2] + $this->ColumnWidth[3], $row, "< = 19", "TLR", 0, 'C');
        $this->Cell($this->ColumnWidth[4] + $this->ColumnWidth[5], $row, "20 - 34", "TLR", 0, 'C');
        $this->Cell($this->ColumnWidth[6] + $this->ColumnWidth[7], $row, "35 - 44", "TLR", 0, 'C');
        $this->Cell($this->ColumnWidth[8] + $this->ColumnWidth[9], $row, "45 and", "TLR", 0, 'C');
        $this->Cell($this->ColumnWidth[10]+$this->ColumnWidth[11]+$this->ColumnWidth[12], $row, "Total", "LR", 0, 'C');
        $this->Cell($this->ColumnWidth[13], $row, "Tabulation", "LR", 1, 'C');
        
        $this->Cell($this->ColumnWidth[0] + $this->ColumnWidth[1], $row, "", "LR", 0, 'C');
        ...
        $this->Cell($this->ColumnWidth[8] + $this->ColumnWidth[9], $row, "above", "LRB", 0, 'C');
        $this->Cell($this->ColumnWidth[10] + $this->ColumnWidth[11] + $this->ColumnWidth[12], $row, "", "LRB", 0);
        $this->Cell($this->ColumnWidth[13], $row, "List", "LR", 1, 'C');
        ...
        $this->Cell($this->ColumnWidth[12], $row, "Total", "LRB", 0, 'C');
        $this->Cell($this->ColumnWidth[13], $row, "", "LRB", 0, 'C');
   ...
   if ($this->dept_nr){
     ...
     $sql = "SELECT cd.code AS subcode, 
        ...
        SUM(CASE WHEN p.sex='f' AND (floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))<=19 OR floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age)) IS NULL) then 1 else 0 end) AS female_below19, 
        '0' AS male_below19, 
        SUM(CASE WHEN p.sex='f' AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))>=20 AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))<=34 then 1 else 0 end) AS female_20to34,   
        '0' AS male_20to34,
        SUM(CASE WHEN p.sex='f' AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))>=35 AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))<=44 then 1 else 0 end) AS female_35to44, 
        '0' AS male_35to44,
        SUM(CASE WHEN p.sex='f' AND floor(IF(fn_calculate_age(p.death_date,p.date_birth),fn_get_age(p.death_date,p.date_birth),age))>=45 then 1 else 0 end) AS female_45above,
        '0' AS male_45above,
        ...
        LEFT JOIN (SELECT ser.encounter_nr,SUBSTRING(MAX(CONCAT(ser.create_time,ser.result_code)),20) AS result_code 
            ...
            WHERE (DATE(discharge_date) BETWEEN '2008-09-01' AND '2008-12-31') 
        ...
        AND (DATE(e.discharge_date) BETWEEN '2008-09-01' AND '2008-12-31') 
