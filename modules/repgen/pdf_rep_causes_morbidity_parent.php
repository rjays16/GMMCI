...
var $t_male_total, $t_female_total, $t_total;
	 ...
	 function RepGen_ResultOfTreatment_Discharge ($from, $to, $dept_nr, $sclass) {
		...
		function FetchData(){
		 ...
		 $sql = "SELECT (SELECT IF(INSTR(p.code_parent,'.'), SUBSTRING(p.code_parent, 1, 3), IF(INSTR(p.code_parent,'/'), SUBSTRING(p.code_parent, 1, 5),p.code_parent))) AS code_parent,
								p.icd_desc AS description,
								...
								ORDER BY count(p.code_parent) DESC LIMIT 50";
