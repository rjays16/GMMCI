...
*/
  ...
  class RepGen_DischargedDiagnosis_Gynecology extends RepGen {
  var $from, $to;
  var $colored = FALSE;

  function RepGen_DischargedDiagnosis_Gynecology ($from, $to) {
    global $db;
  ...
  function Header() {
    ...
    $this->Cell($total_w,4,$row['hosp_addr1'],$border2,1,'C');
     ...
     $this->SetFont('Arial','B',10);
    $this->Cell(17,5);
     $this->Cell($total_w,4,'Leading Causes of Discharges in Gynecology',$border2,1,'C');
...
WHERE cd.diagnosing_dept_nr =124 AND cd.date IS NOT NULL $whereSQL\n GROUP BY ICD10 ORDER BY Total Desc";    
...
$rep = new RepGen_DischargedDiagnosis_Gynecology($_GET['from'], $_GET['to']);
$rep->AliasNbPages();
$rep->FetchData();      
$rep->Report(); 
?>
