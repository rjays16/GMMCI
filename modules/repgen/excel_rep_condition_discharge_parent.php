<?php
	#Causes of Discharges (Group) for DMC
	...
	class ExcelGen_ResultOfTreatment_Discharge extends Spreadsheet_Excel_Writer
	{
			...
			function ExcelGen_ResultOfTreatment_Discharge($from, $to, $dept_nr, $sclass)
			{
			...
			function ExcelHeader()
			{
					...
					$this->worksheet->setHeader("".$row['hosp_country']."\n".$row['hosp_agency']."\n".$row['hosp_name']."\n".$row['hosp_addr1']."\n\n".$deptname." - DISCHARGE DIAGNOSIS (PRIMARY)".$label."\n".$text."\n",0.5);
					...
					$this->worksheet->write(1, 11, $this->Headers[5], $this->format1);
			...
			INNER JOIN care_icd10_en AS en ON en.diagnosis_code = (SELECT IF(INSTR(cd.code_parent,'.'), SUBSTRING(cd.code_parent, 1, 3), IF(INSTR(cd.code_parent,'/'), SUBSTRING(cd.code_parent, 1, 5),cd.code_parent)))
	...
	$rep = new ExcelGen_ResultOfTreatment_Discharge($_GET['from'], $_GET['to'], $_GET['dept_nr_sub'], $_GET['sclass']);
	$rep->ExcelHeader();
	$rep->FetchData();
	$rep->AfterData();
	$rep->send('rep_condition_discharge_parent.xls');
	$rep->close();
?>