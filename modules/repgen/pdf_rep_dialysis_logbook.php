...
//class RepGen_OPD_Trans extends RepGen {
  ...
  function RepGen_ER_Trans($from, $to, $dept_nr) {
    ...
    $this->DMCRepGen("COMPUTERIZED DIALYSIS LOGBOOK", "L", "A4", $db, TRUE);

    $this->Caption = "COMPUTERIZED DIALYSIS LOGBOOK";
  ...
  function Header() {
    ...
    $this->SetFont("Arial","B","12");
        $this->Cell(0,4,'COMPUTERIZED DIALYSIS LOGBOOK',$border2,1,'C');
    #$this->Ln(0);
  ...
  $sql =
        "SELECT DISTINCT ce.encounter_nr, cp.pid,
          ...
          IFNULL(ced.diagnosis_description, sedg.description) AS admitting_diagnosis,
           addr_str,cd.id,(SELECT GROUP_CONCAT(c.firm_id)FROM seg_encounter_insurance b 
          LEFT JOIN care_insurance_firm c ON b.`hcare_id` = c.`hcare_id` WHERE b.encounter_nr = ce.`encounter_nr` ) AS insurance,ce.`discharge_date`,ce.`discharge_time`,
        ...
        FROM (care_encounter AS ce
          ...
          INNER JOIN seg_dialysis_transaction_h sdt ON ce.pid = sdt.pid
