...
var $from, $to;
	 ...
	 function RepGen_ResultOfTreatment_Discharge ($from, $to) {
		...
		$this->Alignment = array('L', 'R','R','R','R', 'R','R','R','R', 'R','R');
	...
	function Header() {
		...
		$this->Cell($total_w,4,'DISCHARGES : SERVICES RENDERED AND PATIENTS ATTENDED',$border2,1,'C');
		...
		$this->Cell(90, 4, "Service", "1", 0, 'C');
				...
				$this->Cell(15, 4, "Discharge", "LR", 0, 'C');
				$this->Cell(20, 4, "Length", "LR", 1, 'C');
				...
				$this->Cell(20, 4, "of Stay", "LRB", 0, 'C');
	...
	function AfterData() {
		...
		if (!$this->_count) {
			...
			$this->Cell(60, $this->RowHeight, "Total =>", 1, 0, 'L', 1);
			$this->Cell(18, $this->RowHeight, number_format($this->total_pay_non_phic), 1, 0, 'R', 1);
			$this->Cell(24, $this->RowHeight, number_format($this->total_pay_phic), 1, 0, 'R', 1);
			$this->Cell(24, $this->RowHeight, number_format($this->total_pay_phic_indigent), 1, 0, 'R', 1);
			$this->Cell(24, $this->RowHeight, number_format($this->total_pay_owwa), 1, 0, 'R', 1);
			$this->Cell(18, $this->RowHeight, number_format($this->total_charity_non_phic), 1, 0, 'R', 1);
			$this->Cell(24, $this->RowHeight, number_format($this->total_charity_phic), 1, 0, 'R', 1);
			$this->Cell(24, $this->RowHeight, number_format($this->total_charity_phic_indigent), 1, 0, 'R', 1);
			$this->Cell(24, $this->RowHeight, number_format($this->total_charity_owwa), 1, 0, 'R', 1);
			$this->Cell(15, $this->RowHeight, number_format($this->total_total_discharge), 1, 0, 'R', 1);
			$this->Cell(20, $this->RowHeight, number_format($this->total_LOS), 1, 1, 'R', 1);
		}
				...
				if ($result) {
					...
					while ($row=$result->FetchRow()) {
								...
								$this->Data[]=array(
										...
										number_format($row['pay_non_phic']),
										number_format($row['pay_phic']),
										number_format($row['pay_phic_indigent']),
										number_format($row['pay_owwa']),

										number_format($row['charity_non_phic']),
										number_format($row['charity_phic']),
										number_format($row['charity_phic_indigent']),
										number_format($row['charity_owwa']),

										number_format($row['total_discharge']),
										number_format($los)
