...
var $location;
	...
	function Header() {
		...
		$this->Cell($total_w,4,'HOSPITAL STATISTICAL REPORT',$border2,1,'C');
			...
			$this->Ln(4);
				...
				switch($this->location){
					...
					case '3':
										...
										break;
					case '4':
										$dist = "OPD Demographic Statistics By Province";
										break;
				}
 ...
 //===============================================================
			...
			### BY PROVINCE
			$sql_loc_prov = "SELECT  prov.prov_name AS Districts, mun.mun_nr,p.mun_nr, mun.mun_name, reg.region_name,
									 SUM(CASE WHEN (soc.discountid!='D' OR soc.discountid IS NULL) then 1 else 0 end) AS pay_non_phic_total,
									 SUM(CASE WHEN i.hcare_id=18 AND (em.memcategory_id<>5 OR em.memcategory_id IS NULL) then 0 else 0 end) AS pay_phic,
									 SUM(CASE WHEN i.hcare_id=18 AND em.memcategory_id=5 then 1 else 0 end) AS pay_phic_indigent,
									 SUM(CASE WHEN em.memcategory_id=3 then 1 else 0 end) AS pay_owwa,
			...
			$result=$db->Execute($sql);
				if ($result) {
				 ...
				 $this->Data[] = array(
										...
										$row['pay_non_phic_total'],
										$row['pay_phic'],
										$row['pay_phic_indigent'],
										$row['pay_owwa'],

										$row['charity_non_phic_total'],
										$row['charity_phic'],
