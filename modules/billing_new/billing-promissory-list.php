 ...
 */
 ...
 if ($_GET["src"]) {
	 ...
	 $smarty->assign('sToolbarTitle',"List of Patients with Promissory Note");
...
# $smarty->assign('pbHelp',"javascript:gethelp('products_db.php','search','$from','$cat')");
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"Lists of Promissory Note");
...
<script language="javascript" type="text/javascript">
	...
	function deletePromi(refno){
		if(confirm("Are you sure you want to delete this promissory?")){
			xajax_removePromi(refno);
		}
	}
</script>
...
# Buffer page output
include($root_path."include/care_api_classes/class_promissory_note.php");

$objpatients = new Promissory_note();
...
$result = $objpatients->getPromiList($filters, $list_rows * $current_page, $list_rows, $keyword);
...
if ($result) {
	...
	while ($row = $result->FetchRow()) {
		...
		$canEdit = "<a title=\"Edit\" href=\"../billing_new/billing-promissory-note.php".URL_REDIRECT_APPEND."&user_origin=billing&encounter_nr=".$row['encounter_nr']."\">
		<img class=\"segSimulatedLink\" src=\"".$root_path."images/cashier_edit.gif\" border=\"0\" align=\"absmiddle\" />
		</a>";

		$canDelete = "<img class=\"segSimulatedLink\" onclick=\"deletePromi('".$row['refno']."');\" src=\"".$root_path."images/cashier_delete.gif\" border=\"0\" align=\"absmiddle\" title=\"Delete\" />";

		$rows .= "<tr class=\"$class\">
					<td align=\"center\">".$row["encounter_nr"]."</td>
					<td>".$row["patient"]."</td>
					<td align=\"center\">".date_format(date_create($row["due_date"]),"M d, Y")."</td>
					<td align=\"right\">".$canEdit.$canDelete."</td>
				</tr>\n";
...
if (!$rows) {
		$records_found = FALSE;
		$rows .= '        <tr><td colspan="10">'.(!$_GET['src'] ? 'No patients found ...' : 'No claims found ...').'</td></tr>';
}
ob_start();
?>
					...
					<td nowrap="nowrap" align="left" class="jedPanel">
						<table width="100%" border="0" cellpadding="2" cellspacing="0">
							...
							<tr>
								...
								<td width="15%" nowrap="nowrap" align="left">Due Date</td>
