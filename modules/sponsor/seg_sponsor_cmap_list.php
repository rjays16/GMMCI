...
$title = "CMAP :: List of CMAP referrals";
...
function initialize() {
	...
	ListGen.create( $('show_requests'), {
		id: 'requests',
		url: '<?=$root_path?>modules/sponsor/ajax/cmap_list.ajax.php',
		params: { 'type':'all'},
		width: 'auto',
		height: 'auto',
		autoLoad: true,
		...
		columnModel: [
			...
			},
			{
				...
				render: function(data, idx){
					var row = data[idx];
					var returnHtml = '<span>'+row.cmapAccount+'</span>';
					if (row.controlNr)
					{
						returnHtml += '<br/><span style="font:normal 11px Tahoma; color:#000066">Control:'+row.controlNr+'</span>';
					}
					return returnHtml;
				}
			},
...
$smarty->assign('sMainBlockIncludeFile','sponsor/cmap_list.tpl');
