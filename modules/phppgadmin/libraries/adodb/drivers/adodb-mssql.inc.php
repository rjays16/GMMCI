...
class ADORecordSet_array_mssql extends ADORecordSet_array {
	...
	global $ADODB_mssql_mths,$ADODB_mssql_date_order;
		...
		if ($ADODB_mssql_date_order == 'dmy') {
			if (!preg_match( "|^([0-9]{1,2})[-/\. ]+([A-Z0-9A-Z0-9]{3})[-/\. ]+([0-9]{4})|" ,$v, $rr)) {
			...
			if (!preg_match( "|^([A-Z0-9A-Z0-9]{3})[-/\. ]+([0-9]{1,2})[-/\. ]+([0-9]{4})|" ,$v, $rr)) {
				return parent::UnixDate($v);
			}
			if ($rr[3] <= TIMESTAMP_FIRST_YEAR) return 0;
	...
	global $ADODB_mssql_mths,$ADODB_mssql_date_order;
		...
		//Dec 30 2000 12:00AM
		 if ($ADODB_mssql_date_order == 'dmy') {
			 if (!preg_match( "|^([0-9]{1,2})[-/\. ]+([A-Z0-9A-Z0-9]{3})[-/\. ]+([0-9]{4}) +([0-9]{1,2}):([0-9]{1,2}) *([apAP]{0,1})|"
			...
			if (!preg_match( "|^([A-Z0-9A-Z0-9]{3})[-/\. ]+([0-9]{1,2})[-/\. ]+([0-9]{4}) +([0-9]{1,2}):([0-9]{1,2}) *([apAP]{0,1})|"
			,$v, $rr)) return parent::UnixTimeStamp($v);
			if ($rr[3] <= TIMESTAMP_FIRST_YEAR) return 0;
