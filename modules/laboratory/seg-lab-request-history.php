...
*/
	...
	function Lab_List_Request ($pid,$encounter_nr) {
		...
		$this->RepGen("PATIENT LABORATORY REQUEST HISTORY","P","Legal");
	...
	function Header() {
		...
		$this->SetFont('Arial','B',10);
		$this->Cell(17,4);
		$this->Cell($total_w,4,'PATIENT LABORATORY REQUEST HISTORY',$border2,1,'C');
        ...
        #added by daryl
        $labwalk_ = $srvObj->selectifwalkin($this->pid);
        $labwalk = $labwalk_->FetchRow();
        $iflabwalk = $labwalk['ifwalk'];
    if ($iflabwalk == 1) {
                $person_ = $srvObj->selectwalkin($this->pid);
                $person_walk = $person_->FetchRow();
                $request_name = $person_walk['name_first']." ".$person_walk['name_middle']." ".$person_walk['name_last'];
                $request_name = ucwords(strtolower($request_name));
                $request_name = htmlspecialchars($request_name);                  
                $request_address = $person_walk['address'];
                $age_="unspecified";
                $enc_nr = "unspecified";
     }  else {
	...
	$request_address = $street_name.$brgy_name.$mun_name.$prov_name;
        ...
        $this->Cell(25,4,$this->pid,$borderNo,$newLineNo,'L');
		...
		$this->SetFont("Arial","B","9");
        $this->Cell(10,4,$age_,$borderNo,$newLineNo,'L');
		$this->Cell(30,4);
		$this->SetFont("Arial","","8");
        ...
        $this->Cell(60,4,$enc_nr,$borderNo,$newLineNo,'L');
		...
		$this->Cell($this->ColumnWidth[5],$row,'ENCODED BY',1,0,'C',1);
