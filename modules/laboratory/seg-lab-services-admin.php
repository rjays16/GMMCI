 ...
 */
 ...
 $smarty->assign('sToolbarTitle',"$LDLab :: Services Manager");
...
// $smarty->assign('pbBack',$returnfile);
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"$LDLab :: Services Manager");

 # Assign Body Onload javascript code

 $onLoadJS='onLoad="preSet(); jsGetServiceGroup(paramselect);"';
...
function startAJAXSearch(searchID, page) {
	...
	if (searchEL) {
		...
		AJAXTimerID = setTimeout("xajax_populateLabServiceList('"+aLabServ+"','"+searchID+"','"+keyword+"',"+page+")",50);
...
function startAJAXSearch2(keyword, page) {
	...
	$("ajax-loading").style.display = "";
	$("ServiceList-body").style.display = "none";
	AJAXTimerID = setTimeout("xajax_populateLabServiceList('"+aLabServ+"','search','"+keyword+"',"+page+")",50);
	lastSearch = keyword;
}
...
function addProductToList(listID, id, ipd_id, opd_id,name, cash, charge, sservice, codenum, available, is_ER, in_lis) {
	var list=$(listID), dRows, dBody, rowSrc;
	var i;
	var classified, mode, editlink, available_label;
	if (list) {
		dBody=list.getElementsByTagName("tbody")[0];
...
//alert('sulod listID, id, name, cash, charge, sservice = '+listID+" , "+id, name+" , "+cash+" , "+charge+" , "+sservice);
		...
		if (id) {
			...
			*/
			...
			var reagent ='onclick="return overlib(OLiframeContent(\'seg-service-reagents.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&popUp='+1+'&service_code='+id+'\', 700, 450, \'flab-list\', 1, \'auto\'), ' +
										'WIDTH, 500, TEXTPADDING, 0, BORDER, 0, STICKY, SCROLL, CLOSECLICK, MODAL, CLOSETEXT, \'<img src=../../images/close.gif border=0 onClick=refreshWindow(key,pagekey);>\', '+
										'CAPTIONPADDING, 4, CAPTION, \'Process Reagents\', MIDX, 0, MIDY, 0, STATUS, \'Process Reagents\');">';

			var reagentlink = '<td align="center"><a href="javascript:void(0);" '+reagent+'<img src="../../images/cashier_edit.gif" border="0"></a></td>';

			if (available==1)
				 available_label = '<td style="color:#007" align="center">Yes</td>';
			else
				 available_label = '<td style="color:#FF0000" align="center">No</td>';

            if (is_ER==1)
                 inER_label = '<td style="color:#007" align="center">Yes</td>';
            else
                 inER_label = '<td style="color:#FF0000" align="center">No</td>';     
                 
            if (in_lis==1)
                 inLIS_label = '<td style="color:#007" align="center">Yes</td>';
            else
                 inLIS_label = '<td style="color:#FF0000" align="center">No</td>';          
			...
			rowSrc = '<tr class="wardlistrow'+alt+'" id="row'+addslashes(id)+'">'+
                            ...
                            '<td align="left"><span style="font:bold 11px Arial;color:#660000">'+ipd_id+'</span></td>'+
                            '<td align="left"><span style="font:bold 11px Arial;color:#660000">'+opd_id+'</span></td>'+
							'<td align="center">'+codenum+'</td>'+
							'<td align="right">'+cash+'</td>'+
							'<td align="right">'+charge+'</td>'+
							'<td style="color:#007" align="center">'+classified+'</td>'+
							/*''+reagentlink+''+*/
							''+available_label+''+
                            ''+inER_label+''+
                            ''+inLIS_label+''+
			...
			rowSrc = '<tr><td colspan="13" style="">No such laboratory service exists...</td></tr>';
		}
...
function AddItem(code,mode){
	...
	return overlib(
				 OLiframeContent('seg-lab-services-edit.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&nr='+escape(code)+"&grpcode="+grpcode+"&mode="+mode, 700, 450, 'fGroupTray', 1, 'auto'),
	...
	<div id="tabFpanel">
		<div align="center" style="display:">
			<table width="100%" cellpadding="4" border="0">
				<tr>
					<td width="28%"><span><strong>Laboratory Section</strong></span></td>
					<td>
				...
				<tr>
					...
					<td>
						<!--<input id="search" name="search" class="segInput" type="text" disabled style="width:50%; margin-left:10px; font: bold 12px Arial" align="absmiddle" onkeyup="if (this.value.length >= 3) startAJAXSearch(this.id,0)" /> -->
						...
						<!--<input id="search" name="search" class="segInput" type="text" size="30" style="font:bold 13px Arial" disabled align="absmiddle" onkeyup="if (this.value.length >= 3) startAJAXSearch(this.id,0)" />-->
...
<table id="ServiceList" class="segList" width="100%" border="0" cellpadding="0" cellspacing="0">
	<thead>
		<tr>
			<th width="1%"></th>
			<th width="*" align="left">Services</th>
			<!--<th width="10%" align="left">Code</th>-->
            <th width="10%" align="left">IPD Code</th>
            <th width="10%" align="left">OPD Code</th>
			<th width="5%" align="left">Code #</th>
			<th width="8%" align="left">Cash</th>
			<th width="8%" align="left">Charge</th>
			<th width="5%" align="left">Discountable</th>
			<!--<th width="5%" align="center">Reagents</th>-->
			<th width="5%" align="center">Available</th>
			<th width="5%" align="center">In ER</th>
	...
	<tbody id="ServiceList-body">
		<tr>
			<td colspan="14">No such laboratory service exists...</td>
		</tr>
	</tbody>
</table>
