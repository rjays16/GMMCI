	...
	function populateLabServiceList($group_code,$sElem,$searchkey,$page) {
		...
		if ($ergebnis) {
			...
			while($result=$ergebnis->FetchRow()) {
				...
				$objResponse->addScriptCall("addProductToList","ServiceList",trim($result["service_code"]),trim($result["ipdservice_code"]),trim($result["oservice_code"]),trim($result["name"]),number_format(trim($result["price_cash"]),2,'.', ''),number_format(trim($result["price_charge"]),2,'.', ''), $result["is_socialized"], $result["code_num"], $available, $result["is_ER"], $result["in_lis"]);
			}#end of while
		} #end of if
	...
	#----------------------
		...
		function populateServiceReagentsList($service_code) {
				...
				if ($ergebnis) {
						...
						while($result=$ergebnis->FetchRow()) {
								#$objResponse->addAlert("name = ".$result['reagent_name']);
								$objResponse->addScriptCall("initialServiceReagentList",$result['reagent_code'],$result['reagent_name'],$result['item_qty'],
																																	 $result['unit_id'],$result['is_unitperpc']);
						}#end of while
				} #end of if
