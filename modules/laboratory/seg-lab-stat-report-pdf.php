   ...
   $pdf->Ln(2);
	...
	if ($totalcount){
		while ($row=$report_info->FetchRow()){
			...
			$report_year3 = $srvObj->getStatReportByYear($row['year'], $datefrom, $dateto);
			
			$report_year4 = $srvObj->getStatReportByYear($row['year'], $datefrom, $dateto);
			$report_year5 = $srvObj->getStatReportByYear($row['year'], $datefrom, $dateto);
			
			$report_year6 = $srvObj->getStatReportByYear($row['year'], $datefrom, $dateto);
			$report_year7 = $srvObj->getStatReportByYear($row['year'], $datefrom, $dateto);
			
			$report_year8 = $srvObj->getStatReportByYear($row['year'], $datefrom, $dateto);
			$report_year9 = $srvObj->getStatReportByYear($row['year'], $datefrom, $dateto);
			
			$report_year10 = $srvObj->getStatReportByYear($row['year'], $datefrom, $dateto);
			$report_year11 = $srvObj->getStatReportByYear($row['year'], $datefrom, $dateto);
			#echo "<br>".$srvObj->sql;
			...
			if ($totalyear){
				...
				$report_group = $srvObj->getLabServiceGroups2();
				$totalgrp = $srvObj->count;
					...
					if ($totalgrp){
							while ($row_mnth=$report_group->FetchRow()){
								...
								for ($i=0; $i<$totalyear;$i++){
									$er_stat_wo = $srvObj->getStatByResultEncType($row_yr['year'], $buf[$row_yr['year']][$i], $fromdate, $todate, $row_mnth['group_code'], $enctype, 0);
									#$total_er_wo_data = $srvObj->getStatByResultEncTypeSum($row_yr['year'], $buf[$row_yr['year']][$i], $fromdate, $todate, $enctype, 0);
									#echo "<br>".$srvObj->sql;
									if ($er_stat_wo['stat_result'])
										$er_wo = $er_stat_wo['stat_result'];
									else
										$er_wo = 0;
									#echo "<br>prev = current =>".$prev." = ". $buf[$row_yr['year']][$i];
									$buf_er_wo[$i] = $er_wo;
									$total_er_wo[$i] = $total_er_wo[$i] + $buf_er_wo[$i];
									#echo "<br>";
									#print_r($buf_er_wo[$i]);
									$pdf->Cell(30,4,$er_wo,"TBLR",0,'R');
									#$prev =  $buf[$row_yr['year']][$i];
								}
								...
								$enctype = '2';
								for ($i=0; $i<$totalyear;$i++){
									$opd_stat_wo = $srvObj->getStatByResultEncType($row_yr['year'], $buf[$row_yr['year']][$i], $fromdate, $todate, $row_mnth['group_code'], $enctype, 0);
								...
								$enctype = '0';
								for ($i=0; $i<$totalyear;$i++){
									$in_stat_wo = $srvObj->getStatByResultEncType($row_yr['year'], $buf[$row_yr['year']][$i], $fromdate, $todate, $row_mnth['group_code'], $enctype, 0);
								...
								$enctype = '3,4';
								for ($i=0; $i<$totalyear;$i++){
									$ipd_stat_wo = $srvObj->getStatByResultEncType($row_yr['year'], $buf[$row_yr['year']][$i], $fromdate, $todate, $row_mnth['group_code'], $enctype, 0);
								...
								$enctype = '1';
								for ($i=0; $i<$totalyear;$i++){
									$er_stat_w = $srvObj->getStatByResultEncType($row_yr['year'], $buf[$row_yr['year']][$i], $fromdate, $todate, $row_mnth['group_code'], $enctype, 1);
								...
								$enctype = '2';
								for ($i=0; $i<$totalyear;$i++){
									$opd_stat_w = $srvObj->getStatByResultEncType($row_yr['year'], $buf[$row_yr['year']][$i], $fromdate, $todate, $row_mnth['group_code'], $enctype, 1);
								...
								$enctype = '0';
								for ($i=0; $i<$totalyear;$i++){
									$in_stat_w = $srvObj->getStatByResultEncType($row_yr['year'], $buf[$row_yr['year']][$i], $fromdate, $todate, $row_mnth['group_code'], $enctype, 1);
								...
								$enctype = '3,4';
								for ($i=0; $i<$totalyear;$i++){
									$ipd_stat_w = $srvObj->getStatByResultEncType($row_yr['year'], $buf[$row_yr['year']][$i], $fromdate, $todate, $row_mnth['group_code'], $enctype, 1);
			...
			while ($row=$report_year3->FetchRow()){
				$totalinfo = $srvObj->getStatByYearMonth($row['year'], $row['month']);
				$pdf->SetFont('Arial','',10);	
