	...
	*/
	...
	if ($totalcount){
			...
			while ($row=$report_info->FetchRow()){
				...
				$pdf->Cell(35,4,$row['serv_dt'].'   '.date("h:i:s A",strtotime($row['serv_tm'])),"",0,'L');
				...
				if ($discountID=='all'){
					...
					*/
					...
					#if (($row['classID'])&&(!($row['type_charge']))){
					#	$classify = $row['classID'];	
					#}elseif (($row['type_charge'])&&(!($row['classID']))){
						#echo "<br>cla2 = ".$row['type_charge'];	
						$classInfo = $srvObj->getChargeTypeInfo($classify);
					#	echo "<br>cla = ".$srvObj->sql;		
						if ($srvObj->count)
							$classify = $classInfo['charge_name'];	
					#}	
					#else
					#	$classify = "NONE";	
					#echo "<br>cla = ".$classify;	
					if ((empty($classify))|| ($classify==" ") || ($classify=="") || !($classify))
						$classify = "NONE";
					#else{
					#if ($classify!="NONE")
		...
		#$pdf->Cell(210,4,"","",0,'L');	
		/*
		$pdf->Cell(25,4,"TOTAL : ","",0,'L');
		
		$pdf->Cell(30,4,number_format($all_total_amount, 2),"",0,'R');	
		$pdf->Cell(2,4,'',"",0,'R');
		$pdf->Cell(30,4,number_format($total_paid, 2),"",0,'R');	
		$pdf->Cell(2,4,'',"",0,'R');
		$pdf->Cell(30,4,number_format($total_amount_bal, 2),"",0,'R');	
		*/
		$pdf->Cell(10,4,"","",0,'L');	
