...
$bill_res = $db->Execute($bill_sql);
while ($row = $bill_res->FetchRow()) {
    $bill_nr = $row['bill_nr'];
}
if ($bill_nr) {
    $cr_sql = "SELECT 
    ...
    $cr_res = $db->Execute($cr_sql);
    while ($row = $cr_res->FetchRow()) {
        if ($row['rate_type'] == 1) {
...
//    $params->put('admitting_diagnosis', $row['admitting_diagnosis']);
//}
...
//
////var_dump($dis_sql);die;
//if (!empty($dis_res)) {
//    while ($row = $dis_res->FetchRow()) {
//        $params->put('discharge_diagnosis', $row['description']);
//    }
//} else {
//    $refno_sql = "SELECT
//                          smo.refno
...
//    $refno_res = $db->Execute($refno_sql);
//    while ($row = $refno_sql->FetchRow()) {
//        $final_sql = "SELECT
//                              smod.description
//                            FROM
//                              seg_misc_ops_details AS smod
//                            WHERE smod.refno =" . $row['refno'] . "";
//        $final_res = $db->Execute($final_sql);
//        while ($row = $final_res->FetchRow()) {
//            $params->put('discharge_diagnosis', $row['description']);
//        }
//    }
//}
...
while ($row = $result->FetchRow()){
    if ($row){
        $params->put('vital_bp', $row['systolic'] ."/". $row['diastolic']);
        $params->put('vital_hr', $row['cr']);
        $params->put('vital_temp', $row['temp']);
        $params->put('vital_rr', $row['rr']);
    }
}
//for general surver
$gs_sql = "SELECT
