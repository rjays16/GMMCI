...
//		}
		...
		$query="SELECT fn_get_pay_account_type(r.ref_source,r.ref_no,r.service_code,r.or_no) `account`,\n".
			"r.or_no, r.amount_due as amount, p.discount_tendered as discount, p.create_id as encoder\n".
		...
		if ($result)
		{
			...
			{
				if(!array_key_exists($row['or_no'], $discount)){
					$discount[$row['or_no']] = $row['discount'];
					$row['amount'] -= $discount[$row['or_no']];
				}
				if($cnt==1)
				{
