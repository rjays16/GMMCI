...
*/
	...
	function RepGen_Cashier_DailyIncomeFull ($typeAcct="", $encoder="", $date_start=FALSE, $time_start=FALSE, $date_end=FALSE, $time_end=FALSE, $or_from=FALSE, $or_to=FALSE, $detailed=FALSE) {
		...
		$this->ColumnLabels = array_merge(
			array('O.R. #', 'DATE/TIME', 'NAME OF PAYOR', 'AMOUNT', 'ENCODER')
		);
...
//		$this->Cell(335.75,3,'',1,1,'C');
		...
		$this->SetFont('Arial','B',10);
		#$this->Cell(19,6);
		$this->Cell($total_w,4,'OPD INCOME REPORT ('.$account.')',$border2,1,'C');
...
/*		$end_date = $this->endDate." ".$this->endTime."0";
	...
	function AfterData() {
		...
		if (!$this->_count) {
			...
			$this->SetFillColor(255);
			$this->SetTextColor(0);
			$this->Cell(20+28+45+12+15, $this->RowHeight, "TOTAL COLLECTION", 1, 0, 'R', 1);
			$this->Cell(32, $this->RowHeight, number_format($this->total_amount_tendered, 2), 1, 0, 'R', 1);
			$this->Cell(43, $this->RowHeight, "", 1, 1, 'L', 1);
		}
				...
				if (!$data[$orNo])
				{
					...
					*/
					...
					$data[$orNo."-".$j] = array_merge(
						array($orNo, $row['or_date'], $row['or_name'], /*$row['ref_source'], $particulars,*/ number_format($amount_due, 2), strtoupper($row['create_id']))
					);
