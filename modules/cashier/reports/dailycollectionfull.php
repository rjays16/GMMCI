...
/*		$end_date = $this->endDate." ".$this->endTime."0";
	...
	function FetchData() {
		...
		$query = "SELECT\n".
				...
				"fn_get_pay_account_type(pd.ref_source, pd.ref_no, pd.service_code, pd.or_no) `account`,\n".
				"(SUM(IF(pay.cancel_date IS NULL,pd.amount_due,0.0)) - pay.discount_tendered) amount,\n".
