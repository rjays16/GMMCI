...
//		xajax_addSelectedEncounter(enc_nr);
		...
		function openInfo(encounter_nr, dr_nr, amount, dr_name){
			return overlib(
         		 OLiframeContent('pf-list-payments.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&popUp=1&encounter_nr='+encounter_nr+"&dr_nr="+dr_nr+"&amount="+amount+"&dr_name="+dr_name, 
		  						500, 250, 'fGroupTray', 0, 'auto'),
          						WIDTH,700, TEXTPADDING,0, BORDER,0, 
									STICKY, SCROLL, CLOSECLICK, MODAL, 
									CLOSETEXT, '<img src=<?=$root_path?>/images/close.gif border=0 onClick="">',
						         CAPTIONPADDING,2, CAPTION,'SET PAYMENTS',
						         MIDX,0, MIDY,0, 
						         STATUS,'SET PAYMENTS');	
		}

		function cancelPayment(refno){
			if(refno){
				var r = confirm("Are you sure you want to cancel this payment?");

				if(r == true){
					xajax_untagPF(refno);
				}
			}else{
				alert("Cannot cancel payment, total excess is zero.");
			}
		}
...
# Buffer page output
include($root_path."include/care_api_classes/class_pf.php");
...
if(!$_GET['refresh']){
	$_POST['chkstatus'] = true;
	$_POST["statusby"] = "unpaid";
}
...
if($_POST['chkdoc']){
	$filters ['doc_name'] = $_POST['doc_name'];
}

if($_POST['chkstatus']){
	switch(strtolower($_POST["statusby"])) {
		case "paid":
				$filters["paid"] = $_POST["paid"];
		break;
		case "unpaid":
				$filters["unpaid"] = $_POST["unpaid"];
		break;
		case "partial":
				$filters["partial"] = $_POST["partial"];
		break;
	}
}
...
if (!$_GET['src'])
		$result = $objpatients->selectList($filters, $list_rows * $current_page, $list_rows, $keyword);
else
		$result = $objpatients->selectList($filters, $list_rows * $current_page, $list_rows, $keyword);
...
if ($result) {
		...
		while ($row = $result->FetchRow()) {
				...
				if ($src_link == "") {
                       ...
                       $balance = $row['excess'] - $row['payment'];
						$balance = $balance <=0 ? "paid" : number_format($balance, 2);
						$rows .= "<tr class=\"$class\">

													<td>".strftime("%Y-%m-%d %I:%M %p", strtotime($row["bill_frmdte"]))."</td>
													<td>".$row["encounter_nr"]."</td>
													<td>".$row["patient"]."</td>";
						if($balance == 'paid'){
							$rows .= "<td>".$row["or_no"]."</td>
									  <td>".$patient_type."</td>
									  <td>".$row['dr']."</td>";
							...
							$rows .= "<td>unpaid</td>
									  ...
									  <td>".$row['dr']."</td>";
													...
													$rows .= "<td align='right'>".number_format($row['excess'],2)."</td>";
													$rows .= "<td align='right'>".$balance."</td>";

													if($balance == 'paid'){
														$rows .= "<td align='center'><img onclick='cancelPayment(".'"'.$row['refno'].'"'.");' style='cursor: pointer;' src = '".$root_path."images/cashier_cancel.gif'></td>";
													}else{
														$rows .= "<td align='center'><img onclick='openInfo(".'"'.$row['encounter_nr'].'","'.$row['dr_nr'].'","'.$row['excess'].'","'.$row['dr'].'"'.");' style='cursor: pointer;' src = '".$root_path."images/cashier_edit.gif'></td>";
													}
								...
								<td nowrap="nowrap" align="left" class="jedPanel">
										<table width="100%" border="0" cellpadding="2" cellspacing="0">
												<tr>
													...
													<tr>
													<td width="50" align="right">
																<input type="checkbox" id="chkstatus" name="chkstatus" onclick="selrecordOnChange(); keepFilters(0);emptier(); disableSearch();" <?= ($_POST['chkstatus'] ? 'checked' : '') ?>/>
														</td>
														<td width="5%" align="left" nowrap="nowrap">Status</td>
														<td>
															<select class="jedInput" name="statusby" id="statusby" onchange="selrecordOnChange(); keepFilters(0);emptier();disableSearch();"/>
																		<option value="unpaid" <?= $_POST["statusby"]=="unpaid" ? 'selected="selected"' : '' ?>>UNPAID</option>
																		<option value="paid" <?= $_POST["statusby"]=="paid" ? 'selected="selected"' : '' ?>>PAID</option>
												...
												<tr>
													<td width="50" align="right">
																<input type="checkbox" id="chkdoc" name="chkdoc" onclick="selrecordOnChange(); keepFilters(0);emptier(); disableSearch();" <?= ($_POST['chkdoc'] ? 'checked' : '') ?>/>
														</td>
														<td width="5%" align="left" nowrap="nowrap">Doctor</td>
														<td>
															<span>
																	<input class="jedInput" name="doc_name" id="doc_name" onblur="keepFilters(0);" onkeyup="disableSearch();" type="text" size="30" value="<?= $_POST['doc_name'] ?>"/>
															</span>	
														</td>
												</tr>
												<tr>
		...
		<div class="segContentPane">
				<table id="" class="jedList" width="100%" border="0" cellpadding="0" cellspacing="0">
						<thead>
								...
								<tr>
										 <th width="15%">Encounter Date</th>
										 <th width="10%">Encounter #</th>
										 <th width="*">Name</th>
										 <th width="9%">Or #</th>
										 <th width="10%">Type</th>
										 <th width="20%">Doctor</th>
										 <th text-align="right" width="5%">Excess</th>
										 <th text-align="right" width="5%">Balance</th>
