...
<script type="text/javascript">
	...
	function showRequest(ref, dept, pid, is_vatable) {
		dept = dept.toLowerCase();
		switch($('mode').value.toLowerCase()) {
			case 'payorrequest':
				if (window.parent.addRequestFromTray) {
					window.parent.addRequestFromTray(dept, ref, is_vatable);
				}
			break
			default:
		...
		switch($('mode').value) {
			...
			case 'payor':
				if ($F('selpayor') == "name") {
					...
					var bvalid = (
									/^[A-Z0-9Ñ\-\.]{2}[A-Z0-9Ñ\-\. ]*\s*,\s*[A-Z0-9Ñ\-\.]{2}[A-Z0-9Ñ\-\. ]*$/.test(strname) ||
		...
		$rows .= "			<tr class=\"$class\" height=\"26\">
			...
			<td class=\"centerAlign\" style=\"white-space:nowrap\">
				...
				(
					...
					"<button name=\"select[]\" id=\"".strtolower($row["source_dept"].$row["reference_no"])."\" class=\"segButton\" onclick=\"showRequest('".$row["reference_no"]."','".$row["source_dept"]."','".$row["request_pid"]."','".$_GET['is_vatable']."'); return false;\"><img src=\"".$root_path."gui/img/common/default/tick.png\"/>Select</button>"
				).
				"
			</td>
			</tr>\n";
...
<input type="hidden" name="isvat" id="isvat" value="<?= $_GET['is_vatable']?>">
