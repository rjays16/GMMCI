...
#	$fontSizeLabel=9;
	...
	$pdf->Text($x+30, $y-5, $row['mun_name']);
...
//1. NAME
	...
	$pdf->Cell(60, 4,strtoupper(stripslashes($name_first)),'', '0','C');

	#$pdf->SetXY($x+60, $y+3);
	$pdf->SetXY($x+65, $y+3);
	#$pdf->MultiCell(40, 4,strtoupper($name_middle), '', 'C','0');
	$name_middle = str_replace(" ","  ",$name_middle);
	$pdf->Cell(40, 4,strtoupper(stripslashes($name_middle)),'', '0','C');

	#$pdf->SetXY($x+100, $y+3);
	$pdf->SetXY($x+105, $y+3);
	#$pdf->MultiCell(40, 4,strtoupper($name_last), '', 'C','0');
	$name_last = str_replace(" ","  ",$name_last);
	$pdf->Cell(41, 4,strtoupper(stripslashes($name_last)),'', '0','C');
...
//4. PLACE OF BIRTH
	...
	#$pdf->Text($x+30, $y+34, "Gonzales Maranan Medical Center Incorporated, DAVAO CITY");
	if ($birth_place_basic)
		$birth_place = mb_strtoupper($birth_place_basic).", ";
	else
		$birth_place = "";

	#$pdf->Text($x+30, $y+34, $birth_place." DAVAO CITY");
	$pdf->Text($x+10, $y+34, mb_strtoupper($birth_place)." ".mb_strtoupper($birth_place_mun));
...
//6. MAIDEN NAME
	...
	$pdf->Cell(55, 4,strtoupper(stripslashes($m_name_first)), '', '0','C');

	#$pdf->SetXY($x+60, $y+69);
	$pdf->SetXY($x+75, $y+69);
	#$pdf->MultiCell(40, 4,strtoupper($m_name_middle), '', 'C','0');
	$pdf->Cell(35, 4,strtoupper(stripslashes($m_name_middle)), '', '0','C');

	#$pdf->SetXY($x+100, $y+69);
	$pdf->SetXY($x+110, $y+69);
	#$pdf->MultiCell(40, 4,strtoupper($m_name_last), '', 'C','0');
	$pdf->Cell(36, 4,strtoupper(stripslashes($m_name_last)), '', '0','C');
...
//13. FATHER'S NAME
	if ((($f_name_first=="N/A") || ($f_name_first=="n/a"))&&(($f_name_middle=="N/A") || ($f_name_middle=="n/a"))&&(($f_name_last=="N/A") || ($f_name_last=="n/a"))){
		...
		if ((stristr($f_name_first,'JR')) || (stristr($f_name_first,'SR'))){
			$father_name = $f_name_first;
		}else{
			$father_name = str_replace(",","",$f_name_first);
		}


		$pdf->Cell(55, 4,strtoupper(stripslashes($father_name)), '', '0','C');

		#$pdf->SetXY($x+60, $y+130);
		$pdf->SetXY($x+65, $y+130);
		#$pdf->MultiCell(40, 4, strtoupper($f_name_middle), '', 'C','0');
		$pdf->Cell(40, 4, strtoupper(stripslashes($f_name_middle)),'', '0','C');

		#$pdf->SetXY($x+100, $y+130);
		$pdf->SetXY($x+105, $y+130);
		#$pdf->MultiCell(40, 4,strtoupper($f_name_last), '', 'C','0');
		$pdf->Cell(41, 4,strtoupper(stripslashes($f_name_last)),'', '0','C');
	}
		...
		if ($is_tribalwed==0){
			...
			if(($parent_marriage_date!='0000-00-00')||($parent_marriage_place)){
				if ($parent_marriage_date!='0000-00-00')
					...
					$parent_marriage_info_tmp = $parent_marriage_place;

				$pdf->Text($x+25, $y+175, 'Tribal Wedding - '.$parent_marriage_info_tmp);
			}else
				$pdf->Text($x+25, $y+175, 'Tribal Wedding');
		}
	}else
...
#		$attendant_date_sign = @formatDate2Local($attendant_date_sign,$date_format);
	 ...
	 if (is_numeric($attendant_name)){
		...
		#$doctor_name = "Dr. ".seg_ucwords(strtolower($doctor_name));
		...
		$pdf->Text($x+20, $y+216, stripslashes($doctor_name));

		$pdf->Text($x+20, $y+221, $attendant_title);

		$pdf->SetXY($x+91, $y+209);
		#$pdf->MultiCell(60, 4,seg_ucwords($attendant_address), '', 'L','0');
...
#		$encoder_date_sign = @formatDate2Local($encoder_date_sign,$date_format);
		...
		$pdf->Text($x+21, $y+270, strtoupper(stripslashes($encoder_name)));
...
#echo "married = ".$is_married;
	if ($is_married!=1){
		...
		if (($f_name_first=="n/a")&&($f_name_first=="n/a")&&($f_name_first=="n/a")){
			...
			if (empty($f_fullname))
								$father = $fname1."".$fname2." ".$f_name_middle.$f_name_last;	//modified by VAN
			else
								$father = $f_fullname;
	...
	if ($officer_date_sign!=""){
		...
		*/
		...
		#$pdf->MultiCell(80, 4,seg_ucwords($officer_address), '', 'L','0');
	...
	if ($is_late_reg){
		...
		if (($nofather)&&(!($late_relationship))){
			...
			if ($late_relationship){
				...
				$father = strtoupper($fname1)." ".strtoupper($f_name_middle)." ".strtoupper($f_name_last)."".strtoupper($fname2);
				*/
				$father = strtoupper($late_husband);
				$pdf->Text($x+121, $y+215, $father);
			}
