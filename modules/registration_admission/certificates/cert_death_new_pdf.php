...
if($deathCertInfo){
    ...
    #Commented by Cherry 01-19-10
    //$x = $pdf->GetX();
    //$y = $pdf->GetY();
    
    #Added by Cherry 01-19-10
    $x = 0;
    $y = 0;
    ...
    #commented by Cherry 01-19-10
    $pdf->Text($x+29, $y+28, "Davao del Sur");
    $pdf->Text($x+29, $y+33, "Digos City");
    #$registry_nr = "123456";
    $pdf->Text($x+152, $y+33, $registry_nr);
...
//1. NAME
    $pdf->SetXY($x+30, $y+43);  
    #$pdf->MultiCell(50, 4,mb_strtoupper($name_first), '', 'L','0');
    $pdf->Cell(40, 4,mb_strtoupper($name_first),'', '0','C');
    
    $pdf->SetXY($x+75, $y+43);    
    #$pdf->MultiCell(40, 4,mb_strtoupper($name_middle), '', 'L','0');
    $pdf->Cell(30, 4,mb_strtoupper($name_middle),'', '0','C');
    
    $pdf->SetXY($x+110, $y+43);    
    #$pdf->MultiCell(40, 4,mb_strtoupper($name_last), '', 'L','0');
    $pdf->Cell(41, 4,mb_strtoupper($name_last),'', '0','C');
...
*/
    ...
    #Added by Cherry 01-20-10
//2. SEX
    if($sex=='m')
        $pdf->Text(15, 43, "Male");
    if($sex=='f')
        $pdf->Text(15, 43, "Female");
...
//3. DATE OF DEATH 
    ...
    $pdf->Text($x+20, $y+57, $deathDay."      ".strtoupper($deathMonthName)."         ".$deathYear);
    
 #Added by Cherry 01-20-10      
//4. DATE OF BIRTH
    $pdf->Text($x+80, $y+57, $birthDay."    ".$birthMonthName."    ".$birthYear); 
 
 #Added by Cherry 01-20-10
//6. PLACE OF DEATH
    $pdf->SetFont("Arial","",$fontSizeInput+2);
    $pdf->Text($x+15, $y+68, "Gonzales Maranan Medical Center Incorporated, DAVAO CITY");
    $pdf->SetFont("Arial","",$fontSizeInput);
 
 #Added by Cherry 01-20-10
//7. CIVIL STATUS
    $pdf->Text($x+155, $y+70, strtoupper($civil_status));  
 
 #Added by Cherry 01-20-10
//8. RELIGION
    $pdf->Text($x+15, $y+80, strtoupper($religion_name));  
...
//9. CITIZENSHIP
    ...
    $pdf->Text($x+70, $y+80, $hdcitz_obj['citizenship']);    
//===========================================
...
//10. RESIDENCE
    ...
    if ((stristr($brgy_row['brgy_name'], 'barangay') === FALSE) && (stristr($brgy_row['brgy_name'], 'brgy') === FALSE)){
        if (!empty($m_address) && !empty($brgy_row['brgy_name']))
            $m_address = $m_address.", ".seg_ucwords(trim($brgy_row['brgy_name']));
        else
            $m_address = $m_address." ".seg_ucwords(trim($brgy_row['brgy_name']));
    }
    
    if (!empty($m_address) && !empty($street_name)){
        $m_address = $m_address.", ".seg_ucwords(trim($brgy_row['mun_name']));
    }else{
        $m_address = $m_address." ".seg_ucwords(trim($brgy_row['mun_name']));
    }
    ...
    if(stristr($brgy_row['mun_name'], 'city') === FALSE){
        if (!empty($m_address)){
            $m_address = $m_address.", ".seg_ucwords(trim($brgy_row['prov_name']));
        }else{
            $m_address = $m_address." ".seg_ucwords(trim($brgy_row['prov_name']));
        }
    }
 ...
 #Added by Cherry 01-20-10
//11. OCCUPATION 
    $pdf->Text($x+15, $y+90, $occupation_name);
        
 #Added by Cherry 01-20-10
//12. NAME OF FATHER
    $pdf->SetXY($x+57, $y+90);
    $pdf->Cell(50,4,strtoupper($father_fname)." ".strtoupper($father_mname)." ".strtoupper($father_lname),0,0,'L');
           
 #Added by Cherry 01-20-10
//13. NAME OF MOTHER
    $pdf->SetXY($x+130, $y+90);
    $pdf->Cell(50,4,strtoupper($mother_fname)." ".strtoupper($mother_mname)." ".strtoupper($mother_maidenname),0,0,'L');  
...
//19d. DEATH BY EXTERNAL CAUSES 
   # A. Manner of death
    ...
    if($death_manner_tmp=='1')
        ...
        //$pdf->Text($x+112, $y+135, "Homicide");   # Homicide   8, 180.5
    else if($death_manner_tmp=='2')
        $manner_of_death = "Suicide";
        //$pdf->Text($x+112, $y+135,"Suicide");   # Suicide   30, 180.5
    else if($death_manner_tmp=='3')
        $manner_of_death = "Accident";
        //$pdf->Text($x+112, $y+135, "Accident");   # Accident   66, 180.5
    else if($death_manner_tmp=='4'){
        #$pdf->Text($x+81, $y+135, "Others");   # Others   100, 180.5
        #$death_manner_info = "sample";
        $manner_of_death = $death_manner_info;
       // $pdf->Text($x+112, $y+135, $death_manner_info);   # specific   132, 180.5
    }
    $pdf->Text($x+112, $y+139, $manner_of_death);    
...
//4. AGE
    ...
    #Commented by 01-20-10    
/*    
//5. PLACE OF DEATH
    $pdf->SetFont("Arial","",$fontSizeInput+2);
    $pdf->Text($x+55, $y+41, "Gonzales Maranan Medical Center Incorporated, DAVAO CITY");
    $pdf->SetFont("Arial","",$fontSizeInput);
*/

    #Commented by Cherry 01-20-10    
/*    
//6. DATE OF DEATH
    $deathYear = date("Y",strtotime($death_date)); 
    $deathMonthName = date("F",strtotime($death_date)); 
...
//8. RESIDENCE
    ...
    if (!empty($m_address) && !empty($brgy_row['mun_name']))
        $m_address = $m_address.", ".seg_ucwords($brgy_row['mun_name']);
    else
        $m_address = $m_address." ".seg_ucwords($brgy_row['mun_name']);
    
    if (!empty($m_address) && !empty($brgy_row['prov_name']))
        $m_address = $m_address.", ".seg_ucwords($brgy_row['prov_name']);
    else
        $m_address = $m_address." ".seg_ucwords($brgy_row['prov_name']);
    */
    /*
    ...
    #$pdf->MultiCell(140, 4,seg_ucwords($m_address), '', 'L','0');
    $pdf->MultiCell(140, 4,$m_address, '', 'L','0');
*/
...
//20. CERTIFICATION OF DEATH
    ...
    $pdf->SetXY($x+6, $y+203.5);    
    #$pdf->MultiCell(65, 4,seg_ucwords($attendant_address), '', 'L','0');
...
//25. INFORMANT
    if (($informant_date_sign!='0000-00-00') && ($informant_date_sign!="")){
        ...
        #$pdf->MultiCell(65, 4,seg_ucwords($informant_address), '', 'L','0');
