...
$pdf->Cell(0,4,'Quezon Ave., Digos City',$border2, 1, 'C');
...
$pdf->MultiCell(180, 6,'            This is to certify that '.stripslashes(strtoupper($name_first)).' '.stripslashes(strtoupper($name_middle)).
					' '.stripslashes(strtoupper($name_last)).' , '.$age.' old, '.stripslashes(strtoupper($sex)).' '.
					', '.mb_strtoupper($civil_status).'  and a resident of '.stripslashes(strtoupper($address)).
...
if ($medCertInfo['is_doc_sig']){
   ...
   if (is_numeric($medCertInfo['dr_nr'])){
	...
	$name_doctor = "Dr. ".$docInfo['name_first']." ".$docInfo['name_2']." ".$dr_middleInitial." ".$docInfo['name_last'];
   		...
   		$name_doctor = "Dr. ".$medCertInfo['dr_nr'];
   }
	//Doctor Name
