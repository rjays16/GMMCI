...
#	$personell_obj=new Personell;
	...
	$pdf->Cell(0,4,'Digos City',$border2,1,'C');
...
#  $pdf->Cell(135,4,'Address ',"TLR",0,'L');
	$pdf->SetFont('Arial','I',$fontSizeLabel);
	$pdf->Cell(15,8,'Address: ',"TL",0,'L');
	#$x = $pdf->GetX();
	#$y = $pdf->GetY();
	#$pdf->SetXY(floor($x), floor($y)-24);
	#$pdf->SetXY($x, $y);
	#$pdf->MultiCell(15, 4, 'Address:', '1', 'J','');
	
	$pdf->SetFont('Arial','',$fontSizeInput);
	#added by VAN 04-30-08
	$x = $pdf->GetX();
	$y = $pdf->GetY();
	#echo 'X = '.$x;
#	echo 'Y = '.$y;
	...
	#$pdf->SetXY($x+15, $y-4);
	$pdf->SetXY($x, $y);
	$pdf->MultiCell(120, 4, $address1, '1', 'J','');
	...
	$y = $pdf->GetY();
	$pdf->SetXY($x+135, $y-8);
	$pdf->MultiCell(27, 4, 'Contact Number', '1', 'J','');
	#$pdf->Cell(27,8,'Contact Number',"TLR",0,'L');
	#$pdf->Cell(7,4,'Sex',"TLR",0,'L');
	$pdf->SetXY($x+162, $y-8);
	$pdf->MultiCell(9, 4, 'Sex', '1', 'J','');
	
	$pdf->SetXY($x+171, $y-8);
	$pdf->MultiCell(19, 4, 'Civil Status', '1', 'J','');
...
#	$pdf->Cell(27,4,"09191234567","LBR",0,'L');
	$pdf->Cell(9,4,mb_strtoupper($sex),"LBR",0,'L');
	$pdf->SetFont('Arial','',$fontSizeInput-2);
	$pdf->Cell(19,4,mb_strtoupper($civil_status),"LBR",1,'L');
