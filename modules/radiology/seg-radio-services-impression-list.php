 ...
 */
 ...
 $smarty->assign('sToolbarTitle',"Radiology :: Impression's Code Manager");
...
// $smarty->assign('pbBack',$returnfile);
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"Radiology :: Impression's Code Manager");
...
function startAJAXSearch(searchID, page) {
	...
	if (searchEL) {
		...
		AJAXTimerID = setTimeout("xajax_populateRadioImpressionList('"+searchID+"','"+searchEL.value+"',"+page+")",50);
...
function startAJAXSearch2(keyword, page) {
	...
	$("ajax-loading").style.display = "";
	$("radiogrouplistTable-body").style.display = "none";
	AJAXTimerID = setTimeout("xajax_populateRadioImpressionList('search','"+keyword+"',"+page+")",50);
	lastSearch = keyword;
}
...
function addRadioFindings(listID, id,code, desc) {
	...
	if (list) {
		...
		if (id) {
			...
			rowSrc = '<tr><td colspan="7">No radiology impression\'s code available at this time...</td></tr>';
		}
...
function deleteRadioFindings(id){
	var answer = confirm("Are you sure you want to delete the radiology impression's code?");
		if (answer){
			xajax_deleteRadioImpression(id);
		}
}
...
function AddItem(id,mode){
	...
	return overlib(
					OLiframeContent('seg-radio-services-impression.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&id='+id+'&mode='+mode, 650, 340, 'fGroupTray', 1, 'auto'),
									...
									CLOSETEXT, '<img src=<?=$root_path?>/images/close.gif border=0 onClick="refreshWindow(key,pagekey);">',
										 CAPTIONPADDING,4, CAPTION,'Add radiological impressions code',
										 MIDX,0, MIDY,0, 
										 STATUS,'Add radiological impressions code');							
}
...
<table id="radiogrouplistTable" class="segList" width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	<tr>
			...
			<th width="15%" align="left">Impression's Code</th>
