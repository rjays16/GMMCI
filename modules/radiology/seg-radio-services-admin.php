...
function deleteService(code){
	var answer = confirm("Are you sure you want to delete the radiological service with a code of "+(code.toUpperCase())+"?");
		if (answer){
...
function startAJAXSearch(searchID, page) {
	...
	var keyword;
	var aRadioServ = $("parameterselect").value;
	...
	if (searchEL) {
		...
		AJAXTimerID = setTimeout("xajax_populateRadioServiceList('"+aRadioServ+"','"+searchID+"','"+keyword+"',"+page+")",50);
...
function startAJAXSearch2(keyword, page) {
	...
	$("ajax-loading").style.display = "";
	$("ServiceList-body").style.display = "none";
	AJAXTimerID = setTimeout("xajax_populateRadioServiceList('"+aRadioServ+"','search','"+keyword+"',"+page+")",50);
	lastSearch = keyword;
}
...
function addProductToList(listID, id, name, cash, charge, sservice, available, is_ER) {
...
//alert('sulod listID, id, name, cash, charge, sservice = '+listID+" , "+id, name+" , "+cash+" , "+charge+" , "+sservice);
		...
		if (id) {
			...
			rowSrc = '<tr><td colspan="10" style="">No such radiological service exists...</td></tr>';
		}
...
function AddItem(code,mode){
	...
	return overlib(
				 OLiframeContent('seg-radio-services-edit.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&nr='+escape(code)+"&grpcode="+grpcode+"&mode="+mode+"&deptnr="+deptnr, 500, 450, 'fGroupTray', 1, 'auto'),
									...
									CLOSETEXT, '<img src=<?=$root_path?>/images/close.gif border=0 onClick="refreshWindow(key,pagekey);">',
										 CAPTIONPADDING,4, CAPTION,label+' radiological service',
										 MIDX,0, MIDY,0,
										 STATUS,label+' radiological service');
}
			...
			<table width="100%" cellpadding="4" border="0">
				<tr>
					...
					<td>
						...
						<select name="dept_nr" id="dept_nr" onChange="jsGetServiceGroup(paramselect);">
				...
				<tr>
					<td width="28%"><span><strong>Radiology Service Section</strong></span></td>
					<td>
						<!--<select name="parameterselect" id="parameterselect" onChange="jsGetRadioService(paramselect,1);">-->
...
<br>
<div  style="display:block; border:1px solid #8cadc0; overflow-y:hidden;overflow-x:hidden; width:75%; background-color:#e5e5e5">
...
<table id="ServiceList" class="segList" width="100%" border="0" cellpadding="0" cellspacing="0">
	<thead>
		...
		<tr>
			...
			<th width="15%" align="left">Cash</th>
			<th width="15%" align="left">Charge</th>
			<th width="13%" align="left">Discountable</th>
	...
	<tbody id="ServiceList-body">
		<tr>
			<td colspan="10">No such radiological service exists...</td>
		</tr>
	</tbody>
</table>
