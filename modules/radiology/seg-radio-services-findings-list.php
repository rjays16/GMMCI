 ...
 */
 ...
 $smarty->assign('sToolbarTitle',"Radiology :: Finding's Code Manager");
...
// $smarty->assign('pbBack',$returnfile);
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"Radiology :: Finding's Code Manager");
...
function startAJAXSearch(searchID, page) {
	...
	if (searchEL) {
		...
		AJAXTimerID = setTimeout("xajax_populateRadioFindingsList('"+searchID+"','"+searchEL.value+"',"+page+")",50);
...
function startAJAXSearch2(keyword, page) {
	...
	$("ajax-loading").style.display = "";
	$("radiogrouplistTable-body").style.display = "none";
	AJAXTimerID = setTimeout("xajax_populateRadioFindingsList('search','"+keyword+"',"+page+")",50);
	lastSearch = keyword;
}
...
function addRadioFindings(listID, id,code, desc, impression) {
	...
	if (list) {
		...
		if (id) {
			...
			rowSrc = '<tr class="wardlistrow'+alt+'" id="row'+addslashes(id)+'">'+
								...
								'<td>'+impression+'</td>'+
								'<td>'+info+'</td>'+
								'<td align="center"><img name="delete'+id+'" id="delete'+id+'" src="../../images/delete.gif" style="cursor:pointer" border="0" onClick="deleteRadioFindings(\''+id+'\');"/></td>'+
								'<td>&nbsp;</td>'+
								'</tr>';				
		}
		else {
			rowSrc = '<tr><td colspan="7">No radiology finding\'s code available at this time...</td></tr>';
		}
...
function deleteRadioFindings(id){
	var answer = confirm("Are you sure you want to delete the radiology finding's code?");
		if (answer){
			xajax_deleteRadioFindings(id);
		}
}

function removeRadioFinding(id) {
	//alert('remove');
...
function AddItem(id,mode){
	...
	return overlib(
					OLiframeContent('seg-radio-services-findings.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&id='+id+'&mode='+mode, 650, 340, 'fGroupTray', 1, 'auto'),
											WIDTH,650, HEIGHT,340, TEXTPADDING,0, BORDER,0, 
									STICKY, SCROLL, CLOSECLICK, MODAL, 
									CLOSETEXT, '<img src=<?=$root_path?>/images/close.gif border=0 onClick="refreshWindow(key,pagekey);">',
										 CAPTIONPADDING,4, CAPTION,'Add radiological findings code',
										 MIDX,0, MIDY,0, 
										 STATUS,'Add radiological findings code');							
}
...
<table id="radiogrouplistTable" class="segList" width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	<tr>
			...
			<th width="15%" align="left">Finding's Code</th>
			<th width="*" align="center">Description</th>
			<th width="1%" align="left">&nbsp;</th>
			<th width="20%" align="center">Impression</th>
