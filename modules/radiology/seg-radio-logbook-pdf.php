...
class Report_Radio extends DMCRepGen {
    ...
    function Report_Radio($fromdate,$todate,$request_status,$pat_type,$rpt_group, $radtech, $doctor_nr, $is_alphabetical, $status_report) {
        ...
        $this->ColumnWidth = array(8,16,20,25,10,7,13,20,11,25,13,25,30,15,30,15,19,19);
        $this->Columns = 19;
        ...
        $this->Alignment = array('C','C','C','C','C','C','C','C','C','C','C','C','C','C','C','C');
    ...
    function FetchData() {
       ...
       if ($this->rpt_group!=0)
        ...
        $sql =  "SELECT h.pid, fn_get_person_name(h.pid) AS patient_name, 
                    IF(fn_calculate_age(NOW(),p.date_birth),fn_get_age(NOW(),p.date_birth),age) AS age,
                    d.service_code, s.NAME AS service_name,
                    dp.name_formal, dp.name_short,
                    IF(d.is_served, 'Yes', 'Not Yet') AS is_served,
                    (SELECT CONCAT(cp_2.name_last, ', ',cp_2.name_first)
                    FROM care_personell AS cpl_2
                    INNER JOIN care_person AS cp_2 ON cp_2.pid=cpl_2.pid
                    WHERE cpl_2.nr = d.rad_tech) AS radtech, d.rad_tech,
                    CONCAT(h.request_date,' ', h.request_time) AS request_date, rd.rid,
                    f.doctor_in_charge, 
                    ...
                    e.encounter_type, IF(h.is_cash,'Cash','Charge') AS iscash, p.sex, date_birth,
                    IF ((f.batch_nr && d.STATUS='done'),'Official',IF(f.batch_nr && d.STATUS='pending','Initial','None')) AS result,
                     e.`current_room_nr` AS room_nr,
                     d.`price_cash_orig` AS price,
                     spay.`or_no` AS or_number
