...
#echo "mode2 = ".$_POST['mode'];
 if (isset($_POST['mode'])){
	...
	if ($mode=='save')
		$codeexist = $srvObj->codeExists($_POST['code']);
	else
		$codeexist = 0;
	...
	#echo serialize($_POST['desc'])." her = ".$_POST['desc'];     //celsy
		...
		if ($srvObj->saveRadioServiceImpression(addslashes($_POST['desc']), $_POST['code'], $id,$_POST['department_nr'], $_POST['mode'])) {	
			if ($mode=='save')
...
<form ENCTYPE="multipart/form-data" action="<?=$thisfile?>" method="POST" name="inputgroupform" id="inputgroupform">
	...
	<?php 
			...
			if (!$id){
					if ($mode=='update'){
					$row = $srvObj->getAllRadioImpressionInfo($id);
