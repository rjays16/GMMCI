...
*/
	...
	function Radio_List_Request ($pid,$encounter_nr) {
		global $db;
		#$this->RepGen("PATIENT REQUEST HISTORY","P",array(215.9,93.13));
		$this->RepGen("PATIENT RADIOLOGY REQUEST HISTORY","P","Legal");
	...
	function Header() {
		...
		$this->SetFont('Arial','B',10);
		$this->Cell(17,4);
		$this->Cell($total_w,4,'PATIENT RADIOLOGY REQUEST HISTORY',$border2,1,'C');
	...
	function FetchData($pid,$encounter_nr) {
		global $db;
		$srvObj=new SegRadio;

		$requestObj = $srvObj->getAllRequestByPid($pid,$encounter_nr);
		#echo "sql = ".$srvObj->sql;
...
$iss = new Radio_List_Request($pid,$encounter_nr);
$iss->AliasNbPages();
$iss->FetchData($pid,$encounter_nr);
$iss->Report();
