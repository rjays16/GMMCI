 ...
 */
 ...
 $smarty->assign('sToolbarTitle',"Radiology :: Radiology Group Manager");
...
// $smarty->assign('pbBack',$returnfile);
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"Radiology :: Radiology Group Manager");
 ...
 $smarty->assign('sOnLoadJs','onLoad="jsGetServiceGroup(suchform);"');
...
function startAJAXSearch(searchID, page) {
	...
	if (searchEL) {
		...
		AJAXTimerID = setTimeout("xajax_populateRadioGroupList('"+searchID+"','"+searchEL.value+"',"+$F('dept_nr')+","+page+")",50);
...
function startAJAXSearch2(keyword, page) {
	...
	$("ajax-loading").style.display = "";
	$("radiogrouplistTable-body").style.display = "none";
	AJAXTimerID = setTimeout("xajax_populateRadioGroupList('search','"+keyword+"',"+$F('dept_nr')+","+page+")",50);
	lastSearch = keyword;
}
...
function addRadioGroup(listID, id, groupName, groupOtherName, dept, dept_nr) {
	var list=$(listID), dRows, dBody, rowSrc;
	var i;
	
	//alert('js : listID, id, groupId, groupName, dept = '+listID+" , "+id+" , "+groupId+" , "+groupName+" , "+dept);
	//alert('id = '+addslashes(id));
	//id = id.replace("^","'");
	//alert('id2 = '+id);
	if (list) {
	   dBody=list.getElementsByTagName("tbody")[0];
		...
		if (id) {
			...
			//var info = '<a href="javascript:void(0);" onclick="AddItem(\''+addslashes(id)+'\','+dept_nr+',\'update\');"><img src="../../images/edit.gif" border="0"></a>';
			var info = '<a href="javascript:void(0);" onclick="AddItem(\''+addslashes(id)+'\',\'update\','+dept_nr+');"><img src="../../images/edit.gif" border="0"></a>';

			rowSrc = '<tr class="wardlistrow'+alt+'" id="row'+addslashes(id)+'_'+dept_nr+'">'+
								'<td width="1%">&nbsp;</td>'+
								'<td width="45%">'+groupName+'</td>'+
								'<td width="28%">'+groupOtherName+'</td>'+
								'<td width="35%">'+dept+'</td>'+
             				'<td width="5%" align="center">'+info+'</td>'+
								'<td width="5%" align="center"><img name="delete'+addslashes(id)+'" id="delete'+id+'" src="../../images/delete.gif" style="cursor:pointer" border="0" onClick="deleteRadioGroup(\''+addslashes(id)+'\','+dept_nr+');"/></td>'+
								'<td width="1%">&nbsp;</td>'+
								'</tr>';				
		}
		else {
			rowSrc = '<tr><td colspan="7">No radiology groups available at this time...</td></tr>';
		}
...
function deleteRadioGroup(id, dept_nr){
	var answer = confirm("Are you sure you want to delete the radiology group?");
		if (answer){
			xajax_deleteRadioGroup(id, dept_nr);
		}
}

function removeRadioGroup(id, dept_nr) {
	//alert('remove');
	var table = document.getElementById("radiogrouplistTable");
	var rowno;
	var rmvRow=document.getElementById("row"+id+'_'+dept_nr);
	if (table && rmvRow) {
		rowno = 'row'+id;
...
//function AddItem(id,dept_nr,mode){
function AddItem(id,mode,dept_nr){
	...
	return overlib(
          OLiframeContent('seg-radio-services-groups.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&id='+escape(id)+'&mode='+mode+'&dept_nr='+dept_nr, 350, 200, 'fGroupTray', 1, 'auto'),
						         ...
						         CAPTIONPADDING,4, CAPTION,caption+' radiological service group',
						         MIDX,0, MIDY,0, 
						         STATUS,caption+' radiological service group');							
}

//added by VAN 03-17-08
function jsGetServiceGroup(d){
	//xajax_getServiceGroup($F('dept_nr'));
	//alert('dept = '+$F('dept_nr'));
	//xajax_populateRadioGroupList('search','*',$F('dept_nr'),0);
	//startAJAXSearch2('*', 0);
	startAJAXSearch('search',0);
}
	...
	<div id="tabFpanel">
		<div align="center" style="display:">
			<table width="100%" cellpadding="4">
				<tr>
					<td width="28%"><span><strong>Radiology Section</strong></span></td>
					<td>
						<?php 
								$all_meds=&$dept_obj->getAllRadiologyDept();
						?>
						<!--<select name="parameterselect" id="parameterselect" onChange="jsGetLabService(this,1);">-->
						<select name="dept_nr" id="dept_nr" onChange="clearSearch(); jsGetServiceGroup(suchform);">
							<option value="0">-All Section-</option>
							<?php 
								if(is_object($all_meds)){
									while($deptrow=$all_meds->FetchRow()){
											echo '<option value="'.$deptrow['nr'].'" ';
											if(isset($dept_nr)&&($dept_nr==$deptrow['nr'])) 'selected';
											echo '>';
				...
				<!--
				<tr>
					<td width="28%"><span><strong>Radiology Service Group</strong></span></td>
					<td>
...
<div style="display:block; border:1px solid #8cadc0; overflow-y:scroll; overflow-x:hidden; height:305px; width:95%; background-color:#e5e5e5">
<table id="radiogrouplistTable" class="segList" width="100%" border="0" cellpadding="0" cellspacing="0">
	<thead>
		...
		<tr>
			...
			<th width="45%" align="left">Group Name</th>
			<th width="28%" align="left">Other Name</th>
			<th width="35%" align="left">Section</th>
