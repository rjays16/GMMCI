...
function startAJAXSearch(searchID, page) {
	...
	var personell_nr = $('personell_nr').value;
	
	if (searchEL) {
		searchEL.style.color = "#0000ff";
		if (AJAXTimerID) clearTimeout(AJAXTimerID);
		$("ajax-loading").style.display = "";
		AJAXTimerID = setTimeout("xajax_populateInsurance('"+searchID+"','"+searchEL.value+"',"+personell_nr+","+page+")",100);
...
<script type="text/javascript" src="<?=$root_path?>modules/personell_admin/js/accre-insurance-tray-gui.js?t=<?=time()?>"></script>
...
?>
	...
	<table width="98%" cellspacing="2" cellpadding="2" style="margin:0.7%">
		<tbody>
			...
			<tr>
				<td>
					<div style="display:block; border:1px solid #8cadc0; overflow-y:hidden; height:335px; width:100%; background-color:#e5e5e5">
						<table id="product-list" class="segList" cellpadding="1" cellspacing="1" width="100%">
							<thead>
								...
								<tr>
									...
									<th width="0.5" align="left"></th>
									<th width="*" align="left">&nbsp;Insurance Company</th>
									<th width="1" align="center">&nbsp;</th>
									<th width="25%">Accreditation No.</th>
									<th width="2%"></th>
								</tr>
							</thead>
							<tbody>
	...
	<input type="hidden" name="personell_nr" id="personell_nr" value="<?=$_GET['personell_nr']?>">
