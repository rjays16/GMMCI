...
if(isset($mode)&&($mode=='search'||$mode=='paginate')&&isset($searchkey)&&($searchkey)){
	...
	if($mode!='paginate'){
		...
		if(is_numeric($suchwort)) {
			...
			$sql='SELECT ps.status, ps.nr, ps.is_discharged, p.name_last, p.name_first, p.date_birth, p.addr_zip, p.sex,p.photo_filename '.$dbtable.$sql2.$sql3;
			//echo $sql;
...
?>
		...
		<td align='center'><b>
		<?php 
			if($oitem=='status') $flag=TRUE;
			else $flag=FALSE;
		 echo $pagen->SortLink("Status",'status',$odir,$flag); 
