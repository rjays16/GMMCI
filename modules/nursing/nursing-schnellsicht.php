...
//$db->debug=1;
 ...
 $dbtable='care_ward';
  ...
  $sql="SELECT SUM(r.nr_of_beds) AS maxbed
			FROM $dbtable AS w LEFT JOIN care_room AS r 
			    ON (
			      r.ward_nr = w.nr 
...
#edited by VAN 04-080-08
  if($is_today)
   $filter = " AND DATE('$s_date') >= l.date_from AND l.date_to = '$dbf_nodate'";
  else
   $filter = " AND DATE('$s_date') >= l.date_from AND DATE('$s_date') <= DATE('".date('Y-m-d')."') AND (DATE('$s_date') <= l.date_to OR l.date_to = '$dbf_nodate')";

  $sql="SELECT
			    (SELECT COUNT(l.location_nr)
			    ...
			    LEFT JOIN care_encounter ce 
			         ...
			         WHERE
			          ...
			          AND ce.`is_discharged` <> 1 
   					  AND l.status <> 'discharged') maxoccbed,
  				w.nr AS ward_nr
          FROM $dbtable AS w ";

  $sql.=" WHERE  w.is_temp_closed IN ('',0) AND w.status NOT IN ('hide','delete','void','inactive')   AND w.date_create <= DATE('$s_date') ";
