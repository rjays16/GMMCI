...
function belegen(anum, mode){
  //alert(mode);
  if((anum=="lock")&&(!confirm("<?php echo $LDConfirmLock ?>")))  return;

  if (mode=='assignBedwaiting'){
    <?php
        echo '
           urlholder="nursing-station-transfer-save.php?mode=assignBedwaiting&sid='.$sid.'&lang='.$lang.'&rm='.$rm.'&bd='.$bd.'&pyear='.$pyear.'&pmonth='.$pmonth.'&pday='.$pday.'&pn="+anum+"&station='.$s.'&ward_nr='.$wnr.'";
        ';
    ?>
  }else{
    <?php
 ...
 <table border=0 cellspacing=0 cellpadding=1>
   <tr>
	 ...
	 <table border=0 bgcolor=#ffffcc cellspacing=0 cellpadding=10>
		...
		<tr>
    	<td><input type="text" name="searchkey" id="searchkey" size=40 maxlength=40 value=""></td>
			<td>&nbsp;</td>
		</tr>
		<tr>
...
<?php
					...
					while($row=$admission->FetchRow())
					{
						...
						<td><font face=arial size=2>&nbsp;
							<a href="javascript:belegen(\''.$row['encounter_nr'].'\',\'assignBedwaiting\')">
