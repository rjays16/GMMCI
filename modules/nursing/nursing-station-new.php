	...
	if($dblink_ok){
		switch($mode)
		{
			...
			//$db->debug=1;
				...
				if (empty($HTTP_POST_VARS['ward_nr']))
					$ward_nr = $HTTP_SESSION_VARS['ward_nr'];
				else
					$ward_nr=$HTTP_POST_VARS['ward_nr'];

#echo "here nursing-station-new.php : mode=update : ward_nr = '".$ward_nr."' <br> \n";
...
//			$nr_of_beds = $ward_obj->countBeds($ward_nr);
			...
			#$room_rate = $roomInfo['room_rate'];
			$type_nr = $roomInfo['type_nr'];
		}
	}
	#echo "mode = ".$mode;
...
if ($popUp!='1'){
	...
	#$smarty->assign('breakfile','javascript:window.parent.pSearchClose();refreshWindow();');
...
function preSet(){
	...
	//alert(document.getElementById('room_nr').value);
	...
	if (d.accomodation_type_temp[0].checked){
		...
		//alert('payward');
		d.accomodation_type_temp[0].disabled = true;
	}
...
function checkRoomNr(roomNr, mod){
	var wardNr = document.getElementById('ward_nr').value;

	if (wardNr==null)
		wardNr = 0;

	xajax_checkRoomNrExists(wardNr, roomNr, mod);
 /*
...
function checkRoomNrExists(roomNr, exists){
 //if ((roomNr < document.getElementById('room_nr_nxt').value)||(exists==1)){
 if (exists==1){
	//if (exists==1)
		alert('Room number '+roomNr+' is already exists in the database');
	//else
	//	alert('Room number is should be equal or higher than '+document.getElementById('room_nr_nxt').value);
	document.getElementById('room_nr_end').value="";
	document.getElementById('room_nr_start').value="";
...
function checkWardForm(){
	var a_type = document.getElementById('accomodation_type').value;
...
//alert("checkWardForm : a_type = '"+a_type+"'");
    ...
    else if (document.getElementById('ward_id').value=="")
    {
		alert("Please enter the ward id.");
		document.getElementById('ward_id').focus();
		return false;        
    }
    else if ( (a_type=="1")&&(document.getElementById('dept_nr_temp').value=="0") )
    {
		alert("Please select the department of the ward.");
		document.getElementById('dept_nr').focus();
		return false;
    }
    else if ((a_type=="1")&&(document.getElementById('mandatory_excess').value==""))
    { //---------- edited by: shan  --------------
        alert("Please enter the Mandatory Excess.");
        document.getElementById('mandatory_excess').focus();
        return false;           
	}
    else if (document.getElementById('description_temp').value=="")
    {
		alert("Please enter the description of the ward.");
		document.getElementById('description_temp').focus();
		return false;  
	/*
	}else if ( (document.getElementById('ward_rate').value=="")||(isNaN(parseFloat(document.getElementById('ward_rate').value))) || (parseFloat(document.getElementById('ward_rate').value)<=0) ){
		document.getElementById('ward_rate').value = "";
		alert("Please enter the rate of the ward.");
		document.getElementById('ward_rate').focus();
		return false;
	*/
	}
    //else if ( (a_type=="1")&& ((document.getElementById('room_nr').value=="") || (parseInt(document.getElementById('room_nr').value)<=0) ) )
//    {
//		alert("Please enter a Room Number.");
//		document.getElementById('room_nr').focus();
//		return false;
//	}
//    else if ( (a_type=="1")&& ((document.getElementById('nr_of_beds').value=="") || (parseInt(document.getElementById('nr_of_beds').value)<=0)) )
//    {
//		document.getElementById('nr_of_beds').value = "";
//		alert("Please enter the number of beds of the ward.");
//		document.getElementById('nr_of_beds').focus();
//		return false;
//	}
//    else if ( (a_type=="1")&& (document.getElementById('type_nr').value==0))
//    {
//		alert("Please select a Room Type.");
//		document.getElementById('type_nr').focus();
//		return false;
//	}

	document.getElementById('description').value=document.getElementById('description_temp').value;
	document.getElementById('dept_nr').value=document.getElementById('dept_nr_temp').value;
	if (a_type=="2"){
			// given that a_type=2 means Payward Accomodation [in DB]
		document.getElementById('dept_nr').value="0";
...
//alert("checkWardForm : rooms.length ='"+rooms.length+"'\n$('count_room').value='"+$('count_room').value+"'");
		...
		for(var i=0;i<rooms.length;i++){
			...
			/*
			if (rates[i].value==""){
				alert("Please enter the rate of a room "+rooms[i].value+".");
				rates[i].focus();
				return false;
			}
			*/
			if (types[i].value==0)
            {
				alert("Please select a room type "+rooms[i].value+".");
				types[i].focus();
				return false;
			}
		}//end of for loop     
...
//alert("checkWardForm : rooms.length ='"+rooms.length+"'\n$('count_room').value='"+$('count_room').value+"'");
        ...
        document.getElementById('room_nr_start').value = rooms[0].value;
        document.getElementById('room_nr_end').value = rooms[roomslen-1].value;
    }// end of if-stmt "if (a_type=="1")"     

//	return true;

	var dForm = document.getElementById('newstat');
...
//alert("checkWardForm : origFormInfo.length='"+origFormInfo.length+"' \norigFormInfo : \n"+origFormInfo);
	...
	//alert('mode = '+$('mode').value);
	//edited by VAN 04-12-08
	//xajax_saveWardRoom(origFormInfo);
	//alert(origFormInfo);
    
	xajax_saveWardRoom(origFormInfo, document.getElementById('mode').value, $('mandatory_excess').value);
...
//	return true;
	...
	//	alert("isPayward : type = '"+type+"' \n$('count_room').value='"+$('count_room').value+"'");
		document.getElementById('accomodation_type').value=type;
		if(type == "2"){
			...
			//added by VAN 04-11-08
			//alert(document.getElementById('room_nr_start').value);
	...
	function checkOccupants(id, val, type){

			var minNoBeds;
		var lastBed_Occupied;
		var haspatient;

		if (type=='P'){
			minNoBeds = document.getElementById('minNoBedsP'+id).value;
			lastBed_Occupied = document.getElementById('lastbedoccupiedP'+id).value;
			haspatient = document.getElementById('haspatientP'+id).value;
		}else{
			minNoBeds = document.getElementById('minNoBedsC').value;
			lastBed_Occupied = document.getElementById('lastbedoccupiedC').value;
			haspatient = document.getElementById('haspatientC').value;
		}
		//alert(minNoBeds+" > "+val);
		//edited by VAN 10-05-09
		if (haspatient==1){
			//if ((parseInt(minNoBeds) > parseInt(val))&&(val!="")){
			if ((parseInt(lastBed_Occupied) > parseInt(val))&&(val!="")){
				alert('Number of beds must be higher or equal to '+lastBed_Occupied+' beds because this room has an occupants already.');
				if (type=='P'){
					document.getElementById('beds'+id).focus();
					document.getElementById('beds'+id).value= '';
				}else{
					document.getElementById('nr_of_beds').focus();
					document.getElementById('nr_of_beds').value= '';
				}
	...
	function clearRooms(list) {
		if (list) {
			...
			if (dBody) {
				document.getElementById('count_room').value = 0;
				dBody.innerHTML = "";
				return true;
			}
		}
...
//alert("reloadRoomInfo : beds_orig ='"+beds_orig+"' \ninfos_orig ='"+infos_orig+"' \nbeds_orig.length ='"+beds_orig.length+"'");
		for(var i=0;i<beds_orig.length;i++){
			...
			//rates[i].value = rates_orig[i];
			types[i].value = types_orig[i];
			type_nr2[i].value = types[i].value;
		}//end of for loop
...
//alert("appendRoom : dBody : \n"+dBody);
			...
			var beds_orig = new Array(),
				...
				// rates_orig = new Array();
				types_orig = new Array();
				 beds_orig = saveTempRoomInfo(beds);
				 infos_orig = saveTempRoomInfo(infos);
				// rates_orig = saveTempRoomInfo(rates);
				types_orig = saveTempRoomInfo(types);
...
//alert("appendRoom : beds_orig&infos_orig : \n"+msg);
			...
			if (dBody) {
				...
				if (details) {
					...
					var deleteImg = 'src="../../gui/img/common/default/delete.gif" border=0 onClick="removeRoom('+id+');"';
					...
					*/
					...
					src = "\n"+
						...
						'	</td> '+
						'	<td> '+
						'		<input type="text" name="rates[]" id="rates'+id+'" onBlur="trimString(this);formatNumber(this.value,2,this);" size=6 maxlength=10 value="" style="text-align:right"> '+
						'	</td> '+
						'	<td align="center"> '+
						'		<img name="delete'+id+'" id="delete'+id+'" '+deleteImg+' alt="Delete" style="cursor:pointer"> '+
						'	</td> ';
					*/
					<?php
						$rtype=&$ward_obj->getAllRoomType();
						while ($row=$rtype->FetchRow()) {
							$selected='';
							#if ($type_nr==$row['nr'])
							#$selected='selected';
							$option .= '<option value="'.$row['nr'].'" '.$selected.'>'.$row['name'].'</option>';
						}
					?>

					var option = '<?=$option?>';
					selectType = '<select id="type_nr2'+id+'" name="type_nr2[]" onchange="setTypeNrArray(this.value,\''+id+'\');">'+
					...
					src = "\n"+
						...
						'	</td> '+
						'	<td> '+ selectType +
						'      <input type="hidden" name="types[]" id="types'+id+'" onBlur="trimString(this);" value="">'+
						'	</td> '+
						'	<td align="center"> '+
						'		<img name="delete'+id+'" id="delete'+id+'" '+deleteImg+' alt="Delete" style="cursor:pointer"> '+
						'	</td> ';
					src +='</tr>';
					document.getElementById('count_room').value = parseInt(document.getElementById('count_room').value) + 1;
					//document.getElementById('type_nr2'+id).value = document.getElementById('types'+id).value;

					//added by VAN 04-11-08
				}// end of if-stmt 'if (details)'
				else {
					src = "									<tr> "+
							"											<td colspan=\"5\" align=\"center\" bgcolor=\"#FFFFFF\" style=\"color:#FF0000; font-family:'Arial', Courier, mono; font-style:Bold; font-weight:bold; font-size:12px;\"> "+
...
//alert("appendRoom : dBody.innerHTML : \n"+dBody.innerHTML+"'");
				//reloadRoomInfo(beds_orig, infos_orig, rates_orig);   //restore the room info values
				reloadRoomInfo(beds_orig, infos_orig, types_orig);   //restore the room info values
				return true;
			}
		}
		return false;
...
function formatNumber(num,dec,obj) {
...
function addRoom(){
	...
	if (rooms){
		...
		//alert(document.getElementById('room_nr_nxt').value);
		if (roomslen==0){
			...
			if (document.getElementById('mode').value=='update'){
				if (parseInt(rooms[roomslen-1].value) >= parseInt(document.getElementById('room_nr_nxt').value)){
					roomNxtNr = parseInt(rooms[roomslen-1].value)+1;
				}else{
					roomNxtNr = parseInt(document.getElementById('room_nr_nxt').value);
				}
			}else
				roomNxtNr = parseInt(rooms[roomslen-1].value) + 1;
...
//alert("addRoom : \n"+msg);
	...
	if (!(room_no==null)){
		...
		//added by VAN 04-11-08
			...
			*/
			checkRoomNr(room_no,0);
		}

//		alert("addRoom : OK button is clicked & VALID room number!");
	//commentd by VAN 06-24-08
	/*
		var details = new Object;
		details.no = room_no;
		appendRoom(document.getElementById('room-list'),details);
	*/
	}else{
...
function checkRoomNrExists_Payward(roomNr, exists){
	if (exists==1){
		alert('Room number '+roomNr+' is already exists in the database');
		return false;
	}else{
...
function emptyIntialRoomList(){
	clearRooms(document.getElementById('room-list'));
	appendRoom(document.getElementById('room-list'),null);
}
...
function wardIdExists(){
	...
	document.getElementById('ward_id').select();
}
...
function changeMode(newMode,ward_nr){
	...
	if (newMode=='update'){
		document.getElementById('mode').value=newMode;
		document.getElementById('ward_nr').value=ward_nr;
		if (document.getElementById('viewstat'))
			document.getElementById('viewstat').submit();
...
function setTypeNr(objval){
	document.getElementById('type_nr').value = objval;
}

function setTypeNrArray(objval,id){
	document.getElementById('types'+id).value = objval;
}
...
#$smarty->assign('LDRoomNr',$required_fld.'Room Number');               #edited by pol
...
*/
#echo "room = ".$room_nr;
#echo $wardInfo['room_nr_start']." ".$wardInfo['accomodation_type'];
/*
if (($wardInfo['room_nr_start'])&&($wardInfo['accomodation_type']==1)){
	#echo "charity";
	#$room_nr = $wardInfo['room_nr_start'];
}else{
	#echo "payward";
	$room_nr = $roomLast['room_nr_end'] + 1;
}
*/
if (($wardInfo['room_nr_start'])&&($wardInfo['accomodation_type']==2)){
	#echo "payward";
	$room_nr = $roomLast['room_nr_end'] + 1;
}
#echo "room = ".$room_nr;
$room_nr_nxt = $roomLast['room_nr_end'] + 1;

$smarty->assign('segRoomNxtNr','<input type="hidden" name="room_nr_nxt" id="room_nr_nxt" value="'.$room_nr_nxt.'">');
...
# Assign input values
$smarty->assign('segName','<input type="text" name="name" id="name" size=35 maxlength=40 value="'.$wardInfo['name'].'">');
...
$smarty->assign('segWardID','<input type="text" name="ward_id" id="ward_id" size=20 maxlength=40 value="'.$wardInfo['ward_id'].'" '.$readOnly.'> [A-Z0-9,1-0]');
...
$haspatient = $ward_obj->hasPatient($wardInfo['nr']);
#echo $ward_obj->sql;
if (empty($haspatient))
	$haspatient = 0;
else{
	$lastbedoccupied = $ward_obj->getLastBedNr_Occupied($wardInfo['nr']);
}

if (empty($lastbedoccupied))
	$lastbedoccupied = 0;

//if (empty($room_nr)){
//	#$room_nr = $room_nr_nxt;
//	$room_nr = 1;
//	$smarty->assign('segNrOfBeds','<input type="hidden" id="lastbedoccupiedC" name="lastbedoccupiedC" value="0"><input type="hidden" id="haspatientC" name="haspatientC" value="0"><input type="hidden" id="minNoBedsC" name="minNoBedsC" value="'.$nr_of_beds.'"><input type="text" name="nr_of_beds" id="nr_of_beds" onBlur="trimString(this); if( (isNaN(Number(this.value))) || (this.value==\'\')) this.value=\'\'; else this.value=parseInt(this.value);" size=6 maxlength=4 value="'.$nr_of_beds.'">');
//}else{
//	$smarty->assign('segNrOfBeds','<input type="hidden" id="lastbedoccupiedC" name="lastbedoccupiedC" value="'.$lastbedoccupied.'">
//    <input type="hidden" id="haspatientC" name="haspatientC" value="'.$haspatient.'"><input type="hidden" id="minNoBedsC" name="minNoBedsC" value="'.$nr_of_beds.'">
//    <input type="text" name="nr_of_beds" id="nr_of_beds" onBlur="trimString(this); checkOccupants('.$wardInfo['room_nr_start'].', this.value, \'C\'); if( (isNaN(Number(this.value))) || (this.value==\'\')) this.value=\'\'; else this.value=parseInt(this.value);" size=6 maxlength=4 value="'.$nr_of_beds.'">');
//}
...
$smarty->assign('segRoomNr','<input type="hidden" name="room_nr" id="room_nr" onKeyUp="setEndRoomNr(this.value);" onBlur="trimString(this); 
    if( (isNaN(Number(this.value))) || (this.value==\'\')) this.value=\'\'; 
        else {this.value=parseInt(this.value); checkRoomNr(this.value,1);}" onKeyUp="setEndRoomNr(this.value);" size=6 maxlength=4 value="'.$room_nr.'">');    #edited by pol
...
$smarty->assign('segRoomNr','<input type="hidden" name="room_nr" id="room_nr" onKeyUp="setEndRoomNr(this.value);" onBlur="trimString(this); if( (isNaN(Number(this.value))) || (this.value==\'\')) this.value=\'\'; else {this.value=parseInt(this.value); checkRoomNr(this.value,1);}" size=6 maxlength=4 value="'.$room_nr.'">');    #edited by pol

//---------------- shandy ------------------
  $smarty->assign('segMandatory','<input type="text" name="mandatory_excess" id="mandatory_excess" onBlur="trimString(this);" onkeydown="return key_check(event, this.value);"  size=6 value="'.$wardInfo['mandatory_excess'].'">');
//------------------ end shand -------------


#$smarty->assign('segRoomInfo','<input type="text" name="info_room" id="info_room" onBlur="trimString(this);" size=55 maxlength=100 value="'.$info_room.'">');
//$smarty->assign('segRoomInfo','<input type="text" name="info_room" id="info_room" onBlur="trimString(this);" size=55 value="'.$info_room.'">');

#$smarty->assign('segRoomRate','<input type="text" name="rate_room" id="rate_room" onBlur="trimString(this); formatNumber(this.value,2,this);" size=6 value="'.number_format($room_rate,2,".","").'" style="text-align:right">');

#added by VAN 06-11-08
$selectRoomType .= '<select id="type_nr2" name="type_nr2" onchange="setTypeNr(this.value);">';
$selectRoomType .= '<option value="0">--Select Room Type--</option>';
$rtype2=&$ward_obj->getAllRoomType();
 while ($row=$rtype2->FetchRow()) {
	$selected='';
	if ($type_nr==$row['nr'])
		$selected='selected';

	$selectRoomType .= '\t<option value="'.$row['nr'].'" '.$selected.'>'.$row['name'].'</option>\n';
}
 $selectRoomType .= '</select>';
//$smarty->assign('segRoomType',$selectRoomType.'<input type="hidden" name="type_nr" id="type_nr" onBlur="trimString(this);" value="'.$type_nr.'">');
#---------------
/*
...
if ($popUp!='1'){
	...
	#$smarty->assign('sCancel','<a href="javascript:window.parent.pSearchClose(); window.parent.refreshWindow();"><img '.createLDImgSrc($root_path,'cancel.gif','0').' border="0"></a>');
}
/*
...
<input type="hidden" name="mandatory_excess" id="mandatory_excess" value="'.$wardInfo['mandatory_excess'].'">
...
$sTemp='
		<script language="javascript">
			isPayward('.$a_type.');
			changeMode("'.($mode?$mode:'create').'",'.$ward_nr.');
		</script>';
...
$count=0;
$result = $ward_obj->getRoomsData($ward_nr);
#echo "sql = ".$ward_obj->sql;
if ($result){
	...
	foreach ($rows as $i=>$row) {
		if ($row) {
			$haspatient = $ward_obj->roomhasPatient($ward_nr, $row['room_nr']);
			#$rec_count = $ward_obj->count;
			#echo "w = ".$ward_obj->sql;
			$lastbedoccupied = $ward_obj->getLastBedNr_Occupied($ward_nr, $row['room_nr']);
			#echo $ward_obj->sql;
			...
			if ($haspatient){
				$deleteImg = 'src="../../gui/img/common/default/disc_unrd.gif" border=0 alt="Has Patient(s)" title="Has Patient(s)"';
			}else{
				$deleteImg = 'src="../../gui/img/common/default/delete.gif" border=0 onClick="removeRoom('.$row['room_nr'].');" alt="Delete" title="Delete" style="cursor:pointer"';
				$haspatient = 0;
			}
			...
			/*
			$src .=
					'<tr class="wardlistrow'.$alt.'" id="row'.$row['room_nr'].'" style="font-weight:bold;padding:0px">
						<td><font face="verdana,arial" size="2" >
							 <input type="hidden" id="minNoBedsP'.$row['room_nr'].'" name="minNoBedsP'.$row['room_nr'].'" value="'.$row['nr_of_beds'].'">
							<input type="hidden" id="rowID'.$row['room_nr'].'" value="'.$row['room_nr'].'">
							<input type="hidden" name="rooms[]" id="rooms'.$row['room_nr'].'" value="'.$row['room_nr'].'">
							'.$row['room_nr'].'
							</font>
						</td>
						<td align="center">
							<input type="text" name="beds[]" id="beds'.$row['room_nr'].'" onBlur="trimString(this); checkOccupants('.$row['room_nr'].', this.value, \'P\');" size="8" maxlength="3" value="'.$row['nr_of_beds'].'">
						</td>
						<td>
							<input type="text" name="infos[]" id="infos'.$row['room_nr'].'" onBlur="trimString(this);" size=50 maxlength=100 value="'.$row['info'].'">
						</td>
						<td>
							<input type="text" name="rates[]" id="rates'.$row['room_nr'].'" onBlur="trimString(this); formatNumber(this.value,2,this);" size=6 maxlength=10 value="'.number_format($row['room_rate'],2,".","").'" style="text-align:right">
						</td>
						<td align="center">
							<img name="delete'.$row['room_nr'].'" id="delete'.$row['room_nr'].'" '.$deleteImg.'>
						</td>
					</tr>
				';
			*/
			...
			$rtype=&$ward_obj->getAllRoomType();
			$option = '';
			while ($result=$rtype->FetchRow()) {
				$selected='';
				#echo "<br>here = ".$row['type_nr']."==".$result['nr'];
				if ($row['type_nr']==$result['nr'])
					$selected='selected';
				$option .= '<option value="'.$result['nr'].'" '.$selected.'>'.$result['name'].'</option>';
			}

			$selectType = '<select id="type_nr2'.$row['room_nr'].'" name="type_nr2[]" onchange="setTypeNrArray(this.value,\''.$row['room_nr'].'\');">
							<option value="0">Select Room Type</option>
										'.$option.'
						 </select>';
			...
			$src .=
					'<tr class="wardlistrow'.$alt.'" id="row'.$row['room_nr'].'" style="font-weight:bold;padding:0px">
						<td align="center"><font face="verdana,arial" size="2" >
						    <input type="hidden" id="minNoBedsP'.$row['room_nr'].'" name="minNoBedsP'.$row['room_nr'].'" value="'.$row['nr_of_beds'].'">
							<input type="hidden" id="haspatientP'.$row['room_nr'].'" name="haspatientP'.$row['room_nr'].'" value="'.$haspatient.'">
							<input type="hidden" id="lastbedoccupiedP'.$row['room_nr'].'" name="lastbedoccupiedP'.$row['room_nr'].'" value="'.$lastbedoccupied.'">
							<input type="hidden" id="rowID'.$row['room_nr'].'" value="'.$row['room_nr'].'">
							<input type="hidden" name="rooms[]" id="rooms'.$row['room_nr'].'" value="'.$row['room_nr'].'">
							'.$row['room_nr'].'
							</font>
						...
						<td> '.$selectType.'
									<input type="hidden" name="types[]" id="types'.$row['room_nr'].'" onBlur="trimString(this);" value="'.$row['type_nr'].'">
							</td>
						<td align="center">
							<img name="delete'.$row['room_nr'].'" id="delete'.$row['room_nr'].'" '.$deleteImg.'>
						</td>
...
if ($src) $smarty->assign('sRoomItems',$src);
