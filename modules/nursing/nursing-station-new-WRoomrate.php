...
function checkWardForm(){
	var a_type = document.getElementById('accomodation_type').value;// given that a_type=2 means Payward Accomodation [in DB]
...
//alert("checkWardForm : origFormInfo.length='"+origFormInfo.length+"' \norigFormInfo : \n"+origFormInfo);
	...
	xajax_saveWardRoom(origFormInfo, document.getElementById('mode').value);
...
//	return true;
	...
	function checkOccupants(id, val, type){
		...
		if ((minNoBeds > val)&&(val!="")){
			alert('Number of beds must be higher or equal to '+minNoBeds+' beds because this room has an occupants already.');
			if (type=='P'){
...
if (empty($room_nr)){
	$room_nr = $room_nr_nxt;
	$smarty->assign('segNrOfBeds','<input type="hidden" id="minNoBedsC" name="minNoBedsC" value="'.$nr_of_beds.'"><input type="text" name="nr_of_beds" id="nr_of_beds" onBlur="trimString(this); if( (isNaN(Number(this.value))) || (this.value==\'\')) this.value=\'\'; else this.value=parseInt(this.value);" size=6 maxlength=4 value="'.$nr_of_beds.'">');
}else{
	$smarty->assign('segNrOfBeds','<input type="hidden" id="minNoBedsC" name="minNoBedsC" value="'.$nr_of_beds.'"><input type="text" name="nr_of_beds" id="nr_of_beds" onBlur="trimString(this); checkOccupants('.$wardInfo['room_nr_start'].', this.value, \'C\'); if( (isNaN(Number(this.value))) || (this.value==\'\')) this.value=\'\'; else this.value=parseInt(this.value);" size=6 maxlength=4 value="'.$nr_of_beds.'">');
}
