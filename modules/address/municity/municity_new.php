...
if(!isset($mode)){
	...
	switch($mode)
	{
		case 'save':
		{
			...
			$HTTP_POST_VARS['mun_nr'] = $address_municity->getLatestPk(); //added by maimai 12-17-2014
			...
			if(!empty($HTTP_POST_VARS['mun_name'])){
				...
				if($address_municity->addressExists(0,$HTTP_POST_VARS['mun_name'],FALSE,$HTTP_POST_VARS['prov_nr'])){
					...
					/*
				} elseif ($address_municity->zipcodeExists(0,$HTTP_POST_VARS['zipcode'])){
					#
					# Do notification
					#
					$mode='zipcode_exists';*/
                }elseif ($address_municity->CodeExists($HTTP_POST_VARS['code'])) {
                    $mode='code_exists';
				} else {
...
<!-- 
	function check(d){
        ...
        if ((municity_code =="")){
            ...
            d.codetmp.focus();
            return false;
        }else if (municity_code.length!==2) {
            alert("<?php echo "Municipality/City's code should be 2 characters"; ?>");
            d.codetmp.focus();
            return false;
        }
        ...
        d.code.value = municity_code + "000";
		return true;
