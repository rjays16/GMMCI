...
class abstractPHPJasperXML
{
        ...
        protected $group_count= array();
        public $arrayVariable;
        protected $lastrowresult= array();
    ...
    protected function display($arraydata= array(),$y_axis=0,$fielddata=false,$maxheight=0) {
...
//                print_r($arraydata);
  //              echo "<br/>";
                ...
                elseif($this->left($path,22)==  "data:image/jpeg;base64")
                {
                    ...
                    if(array_key_exists("scale_type", $arraydata))
                    {
                        if($arraydata["scale_type"] == "Clip")
                        {
                            ...
                            $cropImage = imagecrop($realImage, array('x' => 0, 'y' => 0, 'width' => $sizedata["width"], 'height' => $sizedata['height']));
                            ob_start();
                            imagepng($cropImage);
    ...
    protected function aggArray($arr= array(),$aggtype='sum')
    {
      $total=0;
      foreach($arr as $i=>$no)
      {
    ...
    protected function subDataset_handler($data= array()){
    ...
    protected function queryString_handler($xml_path= array())
    {
            $this->sql =$xml_path;
...
public function analyse_dsexpression($data= array(),$txt=''){
        $i=0;
        $backcurl='___';
        $singlequote="|_q_|";
        $doublequote="|_qq_|";
  ...
  public function showPlusSymbol()
    ...
    protected function default_handler($xml_path= array()) {
    ...
    protected function checkoverflow($arraydata= array(),$txt="",$maxheight=0)
    {
...
//                
    ...
    public function dbFetchData($query='',$option='')
    {
        ...
        protected function field_handler($xml_path= array())
        {            
        ...
        protected function parameter_handler($xml_path= array()) {
        ...
        protected function variable_handler($xml_path= array())
        {
            $name=(string)$xml_path['name'];
...
protected function convertDigit($digit=0)
{
  ...
  protected function setDisplayImageSize($arraydata= array(), $imgdata='')
  {
    if(array_key_exists("scale_type", $arraydata))
     {
    ...
    protected function staticText($xml_path) {
     ...
     protected function showBarcode($data= array(),$y=0)
    {
    ...
    protected function drawBorder($data)
    {
         $topattr=$data->box->topPen->attributes();
          ...
          // echo "<br/><br/>";
              $borderstyles=array(
                      'T'=>array('width'=>$topattr["lineWidth"],'style'=>(string)$topattr["lineStyle"], 'hexcolor'=>(string)$topattr["lineColor"]),
                      'B'=>array('width'=>$bottomattr["lineWidth"],'style'=>(string)$bottomattr["lineStyle"],'hexcolor'=>(string)$bottomattr["lineColor"]),
                      'L'=>array('width'=>$leftattr["lineWidth"],'style'=>(string)$leftattr["lineStyle"],'hexcolor'=>(string)$leftattr["lineColor"]),
                      'R'=>array('width'=>$rightattr["lineWidth"],'style'=>(string)$rightattr["lineStyle"],'hexcolor'=>(string)$rightattr["lineColor"])
              );
            ...
            $arrborder=array('T','R','B','L');
            $border=array();
            foreach($arrborder as $borderno =>$bordername)
            {
                if($borderstyles[$bordername]["width"]>0)
                {
                    $border[$bordername]= array('width' => $borderstyles[$bordername]["width"],'cap' => 'butt', 'join' => 'miter',
