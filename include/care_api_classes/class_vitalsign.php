...
class SegVitalsign extends Core {
		...
		*/
		function get_latest_vital_signs($pid, $encounter_nr) {
			global $db;
			$this->sql = "SELECT systole, diastole, weight, height, temp, resp_rate, pulse_rate FROM $this->vital_tb WHERE
		...
		function deleteVitalSignEncounter($encounter_nr){
			global $db;

			$this->sql = "DELETE FROM seg_encounter_vitalsigns WHERE encounter_nr='$encounter_nr'";
			$result = $db->Execute($this->sql);
				if($result)
						return $result;
				else return 0;
