...
class GlobalConfig  extends Core{
	...
	*/
     ...
     * @return value from care_config_global table
     */
	function getRVSeclaims(){
        global $db;
	        $sql = "SELECT 
	                  ccg.value AS codes
	                FROM
	                  `care_config_global` AS ccg 
	                WHERE ccg.`type` = 'rvs_code_cataract_eclaims' ";
	...
	/**
     * Using global config for dynamic values of recovered codes.
     * @author Jeff Ponteras - 03/16/18
     * @return value from care_config_global table
     */
    function getRecoveredResult(){
        global $db;
	        $sql = "SELECT 
	                  ccg.value AS codes
	                FROM
	                  `care_config_global` AS ccg 
	                WHERE ccg.`type` = 'billing_recovered_codes' ";
	...
	*/
	...
	/**
     * Using global config for dynamic values of personnel in charge. 
     * @author Jeff Ponteras - 04/25/18
     * @return value from care_config_global table for csf
     */
    function getInCharge(){
        global $db;
	        $sql = "SELECT 
	                ...
	                FROM
	                  `care_config_global` AS ccg 
	                WHERE ccg.`type` = 'eclaims_inCharge' ";
    ...
    /**
     * Using global config for dynamic values of OPD-dept.
     * @author Jeff Ponteras - 03/20/18
     * @return value from care_config_global table
     */
    function getOpdAsu(){
        global $db;
	...
	function getDrSwitch() {
	    global $db;
			if($this->result=$db->Execute("SELECT value FROM $this->tb WHERE type = 'eclaims_accreditation_switch'")) {
            	if ($this->result->RecordCount()) {
					$this->row=$this->result->FetchRow();
               		return $this->row['value'];
				}else{return '0';}
