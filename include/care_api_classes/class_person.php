...
#                        if(defined('SHOW_FIRSTNAME_CONTROLLER')&&SHOW_FIRSTNAME_CONTROLLER){
				...
				$sql_enc    =" AND enc.encounter_type NOT IN ('5','12') ";
				...
				#edited by VAN 04-16-08
						...
						$this->sql= " SELECT SQL_CALC_FOUND_ROWS rd.rid, cp.photo_filename,dep.dependent_pid, ps.nr AS personnelID, cp.civil_status, cp.senior_ID, cp.fromtemp, cp.pid, $old_select cp.name_last,cp.name_first,cp.name_middle,cp.date_birth,cp.addr_zip, cp.sex,cp.death_date,cp.death_encounter_nr, cp.status,cp.street_name,
														sb.brgy_name, sm.zipcode, sm.mun_name, sp.prov_name, sr.region_name,IF(fn_calculate_age(NOW(),cp.date_birth),fn_get_age(NOW(),cp.date_birth),age) AS age, cp.sex, cp.suffix, is_temp_bdate,
														SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.encounter_nr)),20) AS encounter_nr,
                                                        ...
                                                        SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.is_maygohome)),20) AS is_maygohome,
...
function SearchForBilling($searchkey='',$maxcount=100,$offset=0,$oitem='name_last',$odir='ASC',$fname=FALSE, $senc_nr='', $hasOnlyFinalBill=FALSE){
				global $db, $sql_LIKE, $root_path;
				...
				if($hasOnlyFinalBill){
					$sql4 = " AND sbe.is_final = 1 AND (sbe.is_deleted <> 1 OR sbe.is_deleted IS NULL)";
				}
...
#----------- added by LST 06-26-2008 to allow search using encounter_nr ---- EOB ----
				...
				if(isset($oitem)&&!empty($oitem)) $sql3 .= "ORDER BY $oitem $odir";
					 ...
					 $this->sql="SELECT SQL_CALC_FOUND_ROWS tbperson.pid, sbe.bill_dte, fn_billing_compute_net_amount(sbe.bill_nr) - sbe.total_prevpayments as total_bill, spn.refno AS promi_refno,
						...
						"tbperson.date_birth, tbperson.sex, "/*MAX(tbenc.encounter_date) AS er_opd_datetime,*/."IFNULL(MAX(tbenc.admission_dt),MAX(tbenc.encounter_date)) AS er_opd_datetime,".
						"CONCAT(IFNULL(MAX(tbenc.admission_dt),MAX(tbenc.encounter_date)),' to present') AS confine_period, ".
						...
						"LEFT JOIN seg_provinces AS sp ON sp.prov_nr=sm.prov_nr\n".
						"LEFT JOIN seg_regions AS sr ON sr.region_nr=sp.region_nr\n".
						"LEFT JOIN seg_billing_encounter as sbe on sbe.encounter_nr = tbenc.encounter_nr\n".
						"LEFT JOIN seg_promissory_note spn
    						ON (spn.encounter_nr = tbenc.encounter_nr AND spn.note_status <> 'deleted')\n".
						$sql2.' '.$sql4.' '.$sql3;
...
#exit();
	...
	$params = array_map("utf8_encode", $params);
        ...
        $mlname = utf8_decode($params['member_lname']);
        $mfname = utf8_decode($params['member_fname']);
        $mmname = utf8_decode($params['member_mname']);
        	...
        	if($buf=$db->Execute($checkinsurance_info_sql)) {
				if($buf->RecordCount()) {
					...
					$this->sql = "INSERT INTO seg_insurance_member_info
								...
								".$db->qstr($mlname).",
								".$db->qstr($mfname).",
								".$db->qstr($mmname).",								".$db->qstr($params['suffix']).",
...
/*Added by genz get the encounter type of patient*/
     function getEncounterType($data){
     	global $db;

     	$this->sql = "SELECT encounter_type FROM care_encounter WHERE pid = ".$db->qstr($data['pid'])." AND encounter_nr = ".$db->qstr($data['encounter']);
     	if($this->result=$db->Execute($this->sql)){
     		$row=$this->result->FetchRow();
...
/*Added by genz get the encounter type of patient*/
     function getEncounterTypePreset($data){
     	global $db;
...
*/
     function countRegularDialysis($id = 0){
     	global $db;

     	$this->sql = "SELECT COUNT(id) AS count_trans FROM seg_dialysis_transaction_d WHERE id = ".$db->qstr($id);
     	if($this->result=$db->Execute($this->sql)){
     		$row=$this->result->FetchRow();
     		return $row['count_trans'];
     	}else{
     		return false;
     	}
     	return false;
...
/*  
	Created by Genz 09/18/2014
	For Dialysis regular patient
	update the trans_flag to done
*/
     function saveDoneRegularDialysis($pid = 0){
     	global $db;

     	$this->sql = "UPDATE seg_dialysis_transaction_h SET trans_flag = 'done' WHERE pid='$pid'";
...
/*  
	Created by Genz 09/18/2014
	For Dialysis regular patient
	update the trans_flag to undone
*/
     function saveUnDoneRegularDialysis($pid = 0){
     	global $db;

     	$this->sql = "UPDATE seg_dialysis_transaction_h SET trans_flag = 'active' WHERE pid='$pid'";
...
/*  
	Created by Genz 09/23/2014
	For Dialysis get new dialysis id
*/

    function getNewDialysisId() {
        global $db;

        $s_id = "";

        $strSQL = "SELECT fn_get_new_dialysis_id() AS id";
...
/*  
	Created by Genz 09/23/2014
	For Dialysis get dialysis transactions
*/

 	function getDialysisTrans($data){
 		global $db;

        $pid = "";

        $strSQL = "SELECT pid 
                    FROM seg_dialysis_transaction_h
                    WHERE trans_flag = 'active'
                    AND pid =".$db->qstr($data['pid']);
        if ($result = $db->Execute($strSQL)) {
            if ($result->RecordCount()) {
				while ($row = $result->FetchRow()){
				    $pid = $row['pid'];
...
/*  
	Created by Genz 09/23/2014
	For Dialysis get dialysis transaction id ##################################################
*/
 	function getDialysisTransId($pid = 0){
 		global $db;
 	...
 	/*  
	Created by Genz 09/23/2014
	For Dialysis get dialysis transaction id where trans_flag = 'done'
	Edited By Jarel 11/19/2014
*/
 	function getDialysisTransIdDone($encounter_nr = 0){
 		global $db;
        ...
        $strSQL = "SELECT 
					  a.`id`
					FROM
					  seg_dialysis_transaction_h a 
					  INNER JOIN seg_dialysis_transaction_d b 
					    ON a.`id` = b.`id` 
					  INNER JOIN seg_billing_encounter c 
					    ON b.`bill_nr` = c.`bill_nr` 
					WHERE c.`is_deleted` IS NULL 
					  AND c.`is_final` = '1' 
					  AND a.`trans_flag` = 'done'
					  AND c.`encounter_nr` =".$db->qstr($encounter_nr);
...
/*  
	Created by Genz 09/23/2014
	insert the data in seg_dialysis_transaction_h
*/

	function saveRegularDialysisPatient_h($pid = 0, $user = ''){
		global $db;

		$id = $this->getNewDialysisId();

		$index ="id ,pid, trans_flag, create_dte, create_id";
	   	
	   	$values = $db->qstr($id).", ".$db->qstr($pid).", 'active', NOW(),".$db->qstr($user);
	    
	    $this->sql = "INSERT INTO seg_dialysis_transaction_h ($index) VALUES ($values)";
...
/*  
	Created by Genz 09/23/2014
	insert the data in seg_dialysis_transaction_d
*/

	function saveRegularDialysisPatient_d($hasTransId =0, $entry_no= 0, $bill_nr= 0, $billdate= ''){
		global $db;

		$id = $hasTransId;

		$index ="id , entry_no, bill_nr, session_date";
	   	
	   	$values = $db->qstr($id).", ".$db->qstr($entry_no).", ".$db->qstr($bill_nr).", ".$db->qstr($billdate);
	    
	    $this->sql = "INSERT INTO seg_dialysis_transaction_d ($index) VALUES ($values)";
...
/*
	Added by genz get pid of a patient
	created on 09-24-2014
*/

function getPid($enc = 0){
	global $db;

        $pid = "";

        $strSQL = "SELECT pid FROM care_encounter WHERE encounter_nr = ".$db->qstr($enc);
        if ($result = $db->Execute($strSQL)) {
            if ($result->RecordCount()) {
				while ($row = $result->FetchRow()){
				    $pid = $row['pid'];
...
/*
	Added by genz get encounter list of a regular dialysis patient
	created on 09-24-2014
*/
function getEcounterListDialysis($refno = 0){
	global $db;
        ...
        $this->sql = "SELECT 
					...
					FROM
					  seg_billing_encounter a 
					  INNER JOIN seg_dialysis_transaction_d b 
					    ON b.bill_nr = a.bill_nr 
					WHERE b.`id` = ".$db->qstr($refno);

   		if($result=$db->Execute($this->sql)) {
            if ($result->RecordCount()) {
				while ($row = $result->FetchRow()){
				    $enc_list = $row['enc_list'];
				}
			}
        }
        return $enc_list;
	...
	/*
		Added by genz get Addmission Date of a regular dialysis patient
		created on 09-25-2014
		Modify Query by Jarel 11/19/2014
	*/
	function getEcounterAdmissionDtDialysis($refno = 0){
		global $db;
	...
	Modify Query by Jarel 11/19/2014
	*/
	function getEcounterDischargeDtDialysis($refno = 0){
		global $db;
			//$db->debug = true;
		$refno = $db->qstr($refno);
        $dischargedt = "";
        ...
        $this->sql = "SELECT 
						  CONCAT(b.`discharge_date`,' ',b.`discharge_time`) discharge_date 
						FROM
						  seg_billing_encounter a 
						  INNER JOIN care_encounter b 
						    ON a.`encounter_nr` = b.`encounter_nr` 
	...
	//added by maimai 12-17-2014
	function deleteDialysis_d($billNr){
		global $db;

		$this->sql = "DELETE FROM seg_dialysis_transaction_d WHERE bill_nr = ".$db->qstr($billNr);
	...
	function updatePersonLedgerInfo($data)
	{
		global $db;

		extract($data);

		$amount = $db->qstr(0 - $amount);

		$this->sql = "UPDATE seg_person_ledger_d SET amount = ".$amount.", 
													 modify_date = NOW(), 
													 modify_id = ".$db->qstr($_SESSION['sess_temp_userid'])."
					  WHERE bill_nr = ".$db->qstr($bill_nr)."
					  AND 	description LIKE '%".$or_no."%'";
    ...
    * Set Patient Ledger Info
    * @param array data
    * @return boolean
    */
    function savePersonLedgerInfo($data)
    {
	   	global $db;
	   	...
	   	$index ="refno, entry_date, pid, encounter_nr, 
	   	        bill_nr, entry_type, amount, pay_type, 
	   	        description, create_id, create_date ";
	   	
	   	$values = "(SELECT UUID()) , NOW(), ".$db->qstr($pid).", ".$db->qstr($encounter_nr).",
	   			  ".$db->qstr($bill_nr).", ".$db->qstr($entry_type).", ".$amount.", ".$db->qstr($pay_type)." ,
	   			 ".$db->qstr($description).", ".$db->qstr($_SESSION['sess_temp_userid']).", NOW()";
	    
	    $this->sql = "INSERT INTO seg_person_ledger_d ($index) VALUES ($values)";
    ...
    * Created by Jarel
    * Created on 09/06/2014
    * Delete Patient Ledger Info based on $cond
    * @param array cond
    * @return boolean
    */
    function deletePersonLedgerInfo($cond)
    {
	   	global $db;

	   	$this->sql = "DELETE FROM seg_person_ledger_d WHERE $cond";
    ...
    * Created by Jarel
    * Created on 09/06/2014
    * Get Outstanding Patient's Bill in Person ledger
    * @param string bill_nr
    * @return double amount
    */
    function getPersonOutstandingBill($bill_nr,$area)
    {
	   	global $db;
	   	$bill_nr = $db->qstr($bill_nr);

	   	// $this->sql modified by janjan, fetch only the non cancelled OR is seg_pay. must do INNER JOIN
	   	$this->sql = "SELECT SUM(a.amount) amount 
	   					FROM `seg_person_ledger_d` a
	   					INNER JOIN `seg_pay` p
		   					ON a.`description` LIKE CONCAT('%', p.`or_no` ,'%') 
						    AND p.`encounter_nr` = a.`encounter_nr`
						    AND a.`pid` = p.`pid` 
	   					WHERE a.`bill_nr` = $bill_nr 
	   					AND a.`entry_type` = 'debit'
	   					AND a.`description` LIKE '%$area%'
    ...
    * Created by Jarel
    * Created on 09/06/2014
    * Get Total Amount from Credit Memo in Person ledger
    * @param string bill_nr
    * @return double amount
    */
    function getTotalCRMBill($bill_nr)
    {
	   	global $db;
	   	$bill_nr = $db->qstr($bill_nr);
    ...
    * Check is has save bill in Person Ledger
    * @param string bill_nr
    * @return boolean
    */
    function hasBillSaveInLedger($bill_nr,$area)
    {
	   	global $db;
	   	$bill_nr = $db->qstr($bill_nr);
    ...
    * Created by Jarel
    * Created on 09/08/2014
    * Check is has cashier transaction in Person Ledger
    * @param string or_no
    * @return boolean
    */
    function hasCashierSaveInLedger($or_no)
    {
	   	global $db;
	   	$or_no = $db->qstr($or_no);
    ...
    //Created by EJ 11/06/2014
    function getAdmissionDate($pid) {
		global $db;
		$pid = $db->qstr($pid);
		$this->sql = "SELECT 
    ...
    public function getFullAge($birthdate)
    {
