...
class DRG extends Encounter{
	...
	*/
	...
	var $tb_icpm_default='care_ops301_en';
	#var $tb_icpm_default='seg_ops_rvs';
	/**
	...
	*/
	...
	SELECT d.*, c.diagnosis_code, c.description, m.description AS parent_desc
		FROM care_encounter_diagnosis As d
		LEFT JOIN care_icd10_en as c On d.code=c.diagnosis_code
		Left join care_icd10_en as m on d.code_parent=m.diagnosis_code
		and d.code_parent not in ('deleted','hidden','inactive','void')
		WHERE d.encounter_nr='2007000000'
		and d.group_nr='0'
		order by d.date

	SElect d.*, c.diagnosis_code, c.description
		FROM seg_encounter_icd As d
		LEFT JOIN care_icd10_en as c On d.diagnosis_code=c.diagnosis_code
		and d.status not in ('deleted','hidden','inactive','void')
		WHERE d.encounter_nr='2007500000'
      ...
      */
		...
		//11/16/2013
		//select if primary
	function primaryVerify($encounter)
	{

		global $db;


	$this->sql="SELECT  encounter_nr,type_nr,status 
				FROM care_encounter_diagnosis 
	...
	function getDiagnosisCodes($enc_nr=0,$enc_type=0){
		global $db;

		if(!$this->internResolveEncounterNr($enc_nr)) return FALSE;

		$cond='';
		if ($enc_type!=0) $cond=" AND encounter_type='$enc_type' ";
		$this->sql="SELECT d.*,TRIM(c.diagnosis_code) AS code, IF(d.diagnosis_description IS NULL,TRIM(c.description),TRIM(d.diagnosis_description)) AS diagnosis,  ".
						" d.type_nr AS type, m.description AS parent_desc ".
"
	, (SELECT id FROM care_department AS dept WHERE dept.nr = d.diagnosing_dept_nr) AS diagnosing_dept_id
	, (SELECT name_formal FROM care_department AS dept WHERE dept.nr = d.diagnosing_dept_nr) AS diagnosing_dept_name
	,
	(
	...
	FROM care_encounter_diagnosis AS enc_d2, care_personell AS cpl_2, care_person AS cp_2
	WHERE enc_d2.encounter_nr=d.encounter_nr AND cpl_2.nr = enc_d2.diagnosing_clinician AND cp_2.pid=cpl_2.pid
		AND enc_d2.diagnosing_clinician = d.diagnosing_clinician AND enc_d2.diagnosis_nr = d.diagnosis_nr
	) AS diagnosing_clinician_name
".
						" FROM  $this->tb_diagnosis AS d ".
						" LEFT JOIN $this->tb_diag_codes AS c ON d.code=c.diagnosis_code ".
						" LEFT JOIN $this->tb_diag_codes AS m ON d.code_parent=m.diagnosis_code ".
						" WHERE d.encounter_nr='$enc_nr' AND d.status NOT IN($this->dead_stat,'added') $cond".
						" ORDER BY d.type_nr DESC, d.date ASC ";
#echo "class_drg.php : getDiagnosisCodes : this->sql = '".$this->sql."'<br> \n";
		if($this->result=$db->Execute($this->sql)) {
		    if($this->result->RecordCount()) {
				return $this->result;
		    } else { return FALSE;}
	 ...
	 * return mixed ADODB record object or boolean
	 */
	// note change the tbP_proc_codes -> care_ops301_es
	/*
	SElect d.*, c.code, c.description, m.description AS parent_desc
	FROM care_encounter_procedure As d
	LEFT JOIN care_ops301_en as c On d.code=c.code
	Left join care_ops301_en as m on d.code_parent=m.code
	and d.code_parent not in ('deleted','hidden','inactive','void')
	WHERE d.encounter_nr='2007500004'
	and d.group_nr='0'
	order by d.date

	SElect p.*, c.code, c.description
	FROM seg_encounter_icp As p
	LEFT JOIN care_ops301_en as c On p.procedure_code=c.code
	and p.status not in ('deleted','hidden','inactive','void')
	WHERE p.encounter_nr='2007500000'
	order by p.create_time

	*/

	function getProcedureCodes($enc_nr=0,$enc_type=0){
	    global $db;
		...
		$this->sql="SELECT p.*, TRIM(c.code) AS code, TRIM(c.description) AS therapy, ".
						" p.type_nr AS type, m.description AS parent_desc ".
"
	, (SELECT id FROM care_department AS dept WHERE dept.nr = p.responsible_dept_nr) AS responsible_dept_id
	, (SELECT name_formal FROM care_department AS dept WHERE dept.nr = p.responsible_dept_nr) AS responsible_dept_name
	,
	(
	SELECT CONCAT(cp_2.title,' ',cp_2.name_first,' ', IF(TRIM(cp_2.name_middle)<>'',CONCAT(LEFT(cp_2.name_middle,1),'. '),''), cp_2.name_last) AS fullname
	FROM care_encounter_procedure AS enc_p2, care_personell AS cpl_2, care_person AS cp_2
	WHERE enc_p2.encounter_nr=p.encounter_nr AND cpl_2.nr = enc_p2.responsible_clinician AND cp_2.pid=cpl_2.pid
		AND enc_p2.responsible_clinician = p.responsible_clinician AND enc_p2.procedure_nr = p.procedure_nr
	) AS responsible_clinician_name
".
						" FROM $this->tb_procedure AS p ".
						" LEFT JOIN $this->tb_proc_codes AS c ON p.code=c.code".
						" LEFT JOIN $this->tb_proc_codes AS m ON p.code_parent=m.code ".
						" WHERE p.encounter_nr='$this->enc_nr' AND p.status NOT IN($this->dead_stat) $cond".
						" ORDER BY p.type_nr DESC, p.date ASC";
#echo "getProcedureCodes : this->sql = '".$this->sql."' <br> \n";
		if($this->result=$db->Execute($this->sql)) {
		    if($this->result->RecordCount()) {
				return $this->result;
		    } else { return FALSE;}
	...
	*/
	function ProcedureCodes($grp_nr=0,$enc_nr=0){
		...
		$this->sql="SELECT p.*, c.description, m.description AS parent_desc
							FROM $this->tb_proc_codes AS c,	$this->tb_procedure AS p
									 LEFT JOIN $this->tb_proc_codes AS m ON p.code_parent=m.code
