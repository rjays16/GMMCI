...
class Product extends Core {
	...
	*/
	...
	/**
	* Table name for pharmacy areas
	*/
	var $tb_phareas='seg_pharma_areas';
	...
	*/
	...
	function usePharmaAreas() {
		$this->coretable = $this->tb_phareas;
	}
	...
	*/
	...
	function getProductName($id) {
		...
		$id_str = implode("','",$id);
		$id_str = "('$id_str')";
		$this->sql = "SELECT bestellnum,artikelname FROM care_pharma_products_main WHERE bestellnum IN $id_str";
		$array = array();
		if ($ergebnis=$db->Execute($this->sql)) {
			while ($row=$ergebnis->FetchRow()) {
				$array[$row['bestellnum']] = $row['artikelname']; 
			}
			return $array;
		}
		else return FALSE;
	...
	function getAllPharmaAreas() {
		$this->usePharmaAreas();
		return $this->getAllDataObject();
	}
}
?>
