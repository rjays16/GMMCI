...
class GuiSearchPerson {
	...
	*/
	...
	function display($skey=''){
		...
		global $ptype, $allow_patient_register, $allow_newborn_register, $allow_er_user, $allow_opd_user, $allow_ipd_user, $allow_phs_user, $allow_medocs_user, $allow_update;
...
#echo "enc_row['encounter_type'] = '".$enc_row['encounter_type']."' allow_show_details = '$allow_show_details' <br> \n";
						...
						if ($allow_show_details){
							$sTarget = "<a href=\"$this->targetfile".URL_APPEND."&pid=".$zeile['pid']."&edit=1&status=".$status."&target=".$target."&user_origin=".$user_origin."&noresize=1&ptype=".$ptype."&mode=\">";
							$sTarget=$sTarget.'<img '.createLDImgSrc($root_path,'ok_small.gif','0').' title="'.$LDShowDetails.'"></a>';
						}
						$this->smarty->assign('sOptions',$sTarget);
					}
					...
					if (($totalcount==1)&&($allow_show_details)) {
						...
						$this->smarty->assign('sMiddleName',ucfirst($zeile['name_middle']));
						header("Location:".$this->targetfile.URL_APPEND."&pid=".$zeile['pid']."&edit=1&status=".$status."&target=".$target."&user_origin=".$user_origin."&noresize=1&ptype=".$ptype."&mode=");
						exit();
					}
